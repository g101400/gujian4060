...
PRODUCTS=(
 "水利工程一张图|3.31|D:/Users/Claw/native-shell/win-water-webview2/webroot|D:/Users/Claw/android-build/shuili-v329/app-release.apk"
 "水利感知项目一张图|1.7|D:/Users/Claw/native-shell/win-webview2/webroot|D:/Users/Claw/android-build/perc-v13/app-release.apk"
 "古建景点打卡|1.9|D:/Users/Claw/native-shell/win-gujian-webview2/webroot|D:/Users/Claw/travel/android/app-release.apk"
)
...
build_one() {
  ...
  cp "$APK" "$OUT/${APP}V${VER}_${DATE}_5090_new4060.apk"
  echo "  ✅ ${APP}V${VER}_${DATE}_5090_new4060.apk"

  # 2) UOS DEB
  echo "[2/4] UOS DEB"
  python3 "${SKILL_WIN}/assets/build_deb.py" --version "${VER}-${DATE}" --src "$SRC_WIN" --out "${OUT_WIN}/${DEB_PKG}_${VER}-${DATE}_all_new4060.deb"
  echo "  ✅ ${DEB_PKG}_${VER}-${DATE}_all_new4060.deb"
...
EOF
  ...
  local win_out="$(cygpath -w "$OUT/${APP}V${VER}_${DATE}_Win11_x86_64_一键安装_new4060.exe" 2>/dev/null || echo "$OUT/${APP}V${VER}_${DATE}_Win11_x86_64_一键安装_new4060.exe")"
...
CFG
  ...
  python3 "${SKILL_WIN}/assets/build_apple_zip.py" --version "${VER}_${DATE}" --app "$APP" --src "$SRC_WIN" --out "${OUT_WIN}/${APP}_iOS_${VER}_${DATE}_可托管_new4060.zip"
  echo "  ✅ ${APP}_iOS_${VER}_${DATE}_可托管_new4060.zip"

  # 校验
  echo "[校验] verify_pkg.py"
  python3 "${SKILL_WIN}/assets/verify_pkg.py" \
    --apk "$OUT_WIN/${APP}V${VER}_${DATE}_5090_new4060.apk" \
    --deb "$OUT_WIN/${DEB_PKG}_${VER}-${DATE}_all_new4060.deb" \
    --exe "$OUT_WIN/${APP}V${VER}_${DATE}_Win11_x86_64_一键安装_new4060.exe" \
    --zip "$OUT_WIN/${APP}_iOS_${VER}_${DATE}_可托管_new4060.zip" \
    --version "$VER" || true
