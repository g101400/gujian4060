...
(async () => {
  ...
  const kbBefore = JSON.parse(vm.runInContext("JSON.stringify(window.AIModule._kb ? window.AIModule._kb() : (JSON.parse(localStorage.getItem('ai_kb_shuili')||'{\"index\":[]}').index.length))", ctx));
  ...
  const kbAfterS = vm.runInContext("localStorage.getItem('ai_kb_shuili')", ctx);
