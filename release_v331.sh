...
ROOT="D:/Users/Claw"
DATE="20260901"
OUT_ROOT="${1:-$ROOT/APK归档/四端安装包_${DATE}_v348_智能查询}"
UOS_ARCH="${2:-mips64el}"
...
PRODUCTS=(
"shuili|水利工程一张图|3.48|$ROOT/android-build/shuili-v329|$ROOT/native-shell/win-water-webview2|{A1B2C3D4-0001-4000-8000-000000000001}|水利工程基础信息一张图.exe"
"perc|水利感知项目一张图|1.24|$ROOT/android-build/perc-v13|$ROOT/native-shell/win-webview2|{B2C3D4E5-0002-4000-8000-000000000002}|水利感知项目一张图.exe"
"gujian|古建景点打卡|3.6|$ROOT/travel/android|$ROOT/native-shell/win-gujian-webview2|{C3D4E5F6-0003-4000-8000-000000000003}|古建景点打卡.exe"
)
...
for line in "${PRODUCTS[@]}"; do
  ...
  if [ ! -f "$APK" ]; then
    ...
    cp "$APK" "$OUT/${NAME}V${VER}_${DATE}_5090_new4060.apk"
    note "✅ ${NAME}V${VER}_${DATE}_5090_new4060.apk ($(stat -c%s "$APK") bytes)"
  fi
  ...
  if [ ! -f "$PUB/$EXE" ]; then
    ...
    if [ -f "$NSIS_BIN" ]; then
      python3 "$(cygpath -w "$SKILL_DIR")/assets/build_win_nsis.py" \
        --src "$(cygpath -w "$PUB")" \
        --out "$(cygpath -w "$OUT/${NAME}V${VER}_${DATE}_Win11_x86_64_一键安装_new4060.exe")" \
