...
(function (global) {
  ...
  function pickRecord(dm, cb) {
    ...
    let q = "";
    // v2.4.1：占位符用「请输入名称 / 关键词搜索…」并提供贴近古建的示例 chips（v2.3 之前留空，让用户先看到几枚建议）
    ...
    const render = () => {
      ...
      const html =
        `<div class="field"><input id="pkSearch" class="inp" placeholder="输入名称 / 关键字，按名称/省/市/类型模糊搜" value="${esc2(q)}"></div>` +
  ...
  const AI = {
    ...
    kbOnlineQuery(q) {
      const sys = "你是古建资料助手。基于用户提供的本地资料与可靠信息，回答古建相关查询；如需补充公开常识请注明来源。用中文、简洁、准确。";
