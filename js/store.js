...
(function (global) {
  ...
  // ---------- 打卡记录（古建打卡 APP：独立于 delta，单独持久化）----------
  const CK_KEY = "checkins";
  function ckGet() {
    return tx("readonly").then((os) => new Promise((res, rej) => {
      const r = os.get(CK_KEY);
  ...
  const Checkins = {
    get: ckGet,
    async add(c) { const a = await ckGet(); a.push(c); return ckSet(a); },
    async del(id) { const a = (await ckGet()).filter((x) => x.id !== id); return ckSet(a); },
    async clear() { return ckSet([]); }
  };
