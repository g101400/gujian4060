...
<kml xmlns="http://www.opengis.net/kml/2.2" xmlns:gx="http://www.google.com/kml/ext/2008">
  <Document>
    ...
    <name>${escapeXml(root || "古建基础信息")}</name>
${places}
  </Document>
</kml>`;
  }
  ...
  function xlsxBytesFromMatrix(rows) {
    ...
    var workbook = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' + NL +
      '<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="古建" sheetId="1" r:id="rId1"/></sheets></workbook>';
  ...
  function xlsBytesFromMatrix(rows) {
    ...
    var xml = '<?xml version="1.0"?>' + NL + '<?mso-application progid="Excel.Sheet"?>' + NL +
      '<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><Worksheet ss:Name="古建"><Table>' + body + '</Table></Worksheet></Workbook>';
    return utf8(xml);
  }
  ...
  function matrixToRecords(matrix) {
    ...
    for (var r = 1; r < matrix.length; r++) {
      ...
      out.push({
        ...
        // 古建语义字段：本应用以 city(城市)/atype(古建类型) 为准，导入需回填，否则筛选/列表/ovobj 往返丢失
        city: office, atype: btype,
  ...
  // 文本格式：首行可为中文表头（名称/城市/区县/古建类型/经度/纬度/说明），兼容增删列与超集；
  // 无表头时严格按 `名称,经度,纬度[,城市,古建类型,说明]` 顺序逐行解析。
  // 「奥维原生二进制 .ovobj」（zip/gzip 容器）在 app.js 走 kmz 解包兜底，本函数只负责文本坐标。
  function recordsToOvobj(records, cols) {
    var C = [
      { k: "name", t: "名称", g: function (r) { return r.name || ""; } },
      { k: "city", t: "城市", g: function (r) { return r.city || ""; } },
      { k: "station", t: "区县", g: function (r) { return r.station || ""; } },
      { k: "atype", t: "古建类型", g: function (r) { return r.atype || ""; } },
      { k: "lon", t: "经度", num: true, g: function (r) { return r.lon; } },
      { k: "lat", t: "纬度", num: true, g: function (r) { return r.lat; } },
      { k: "desc", t: "说明", g: function (r) { return (Object.keys(r.params || {}).map(function (x) { return x + " : " + r.params[x]; }).join(NL)) || r.description || ""; } }
    ];
    ...
    var esc = function (s) { return '"' + String(s == null ? "" : s).replace(/"/g, '""') + '"'; };
    var lines = ["# 古建一张图 ovobj 文本坐标文件（名称/城市/区县/古建类型/经度/纬度/说明）",
  ...
  function parseOvobjToRecords(text) {
    ...
    for (var j = 0; j < data.length; j++) {
      ...
      if (isNaN(lon) || isNaN(lat)) continue;
      var name = toks[0], city = toks[3] || "", atype = toks[4] || "", desc = toks.slice(5).join(";");
      out.push({
        id: name + "_" + lon.toFixed(5) + "_" + lat.toFixed(5), name: name, lon: lon, lat: lat,
        office: city, city: city, station: "", atype: atype, btype: atype, type: atype,
  ...
  function parseOvobjBinary(bytes) {
    ...
    for (var k = 0; k < hits.length; k++) {
      ...
      out.push({
        ...
        city: "", office: "", station: "", atype: "", btype: "",
  ...
  global.IO = {
    ...
    // 导出入口
    exportOvkmz(records, root) { downloadBytes((root || "古建基础信息") + ".ovkmz", recordsToKmzBytes(records), "application/vnd.google-earth.kmz"); },
    exportKmz(records, root) { downloadBytes((root || "古建基础信息") + ".kmz", recordsToKmzBytes(records), "application/vnd.google-earth.kmz"); },
    exportKml(records, root) { downloadText((root || "古建基础信息") + ".kml", buildKML(records, root), "application/vnd.google-earth.kml+xml"); },
    exportCsv(records, root, cols) { downloadText((root || "古建基础信息") + ".csv", buildCsv(records, cols), "text/csv;charset=utf-8"); },
    exportChaoheFile(records, root, cols) { downloadText((root || "古建基础信息") + "_潮河.csv", buildChaohe(records, cols), "text/csv;charset=utf-8"); },
    ...
    exportAttrCsv(records, root, cols) { downloadText((root || "古建信息") + ".csv", buildAttrCsv(records, cols), "text/csv;charset=utf-8"); },
    exportXlsx(records, root, cols) { downloadBytes((root || "古建信息") + ".xlsx", buildAttrXlsx(records, cols), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"); },
    exportXls(records, root, cols) { downloadBytes((root || "古建信息") + ".xls", buildAttrXls(records, cols), "application/vnd.ms-excel"); },
    // ovobj/obj 文本坐标（奥维坐标 · 纯文本往返）
    parseOvobjToRecords, recordsToOvobj, parseOvobjBinary, recordsToOvobjBinary,
    exportOvobj(records, root, cols) { downloadText((root || "古建信息") + ".ovobj", recordsToOvobj(records, cols), "text/plain;charset=utf-8"); },
    exportOvobjBin(records, root) { downloadBytes((root || "古建信息") + ".ovobj", recordsToOvobjBinary(records), "application/octet-stream"); },
  };
})(window);
