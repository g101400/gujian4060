...
PRODUCTS = {
    ...
    "gujian": dict(
        # 注意：gujian 的构建真相源已是 travel/android/assets（release_v331.sh /
        # build_ios_zip.py 均从此读取），而非旧的 android-build/gujian-v31/assets。
        # 旧 gujian-v31 曾长期落后于注入版（缺 renderQueryFurther 等），污染同步门禁；
        # 现把 travel/android/assets 设为 canonical，并把 gujian-v31/assets 降为镜像之一
        # （与 shuili 的 canonical→android 镜像 模式一致），使全部分叉副本随 sync 自动对齐。
        canonical=os.path.join(ROOT, "travel", "android", "assets"),
        mirrors=[
            os.path.join(ROOT, "android-build", "gujian-v31", "assets"),
