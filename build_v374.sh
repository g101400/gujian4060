#!/bin/bash
# build_v374.sh — 三产品×四平台 内部通道构建（DATE 锁定 20260913，匹配新构建日期）
# 前置：bump_version_v374_int.py 已跑（version.json/app.js/构建脚本均已 bump，镜像已同步）。
set -u
ROOT="D:/Users/Claw"
ARCHIVE_NAME="${1:-四端安装包_20260913_v374_int}"
V_SHUILI="${2:-3.74}"; V_PERC="${3:-1.50}"; V_GUJIAN="${4:-3.7.12}"
DATE="20260913"
...
declare -A APK_NAMES=( [shuili]="水利工程一张图V${V_SHUILI}_${DATE}_new4060.apk" [perc]="水利感知项目一张图V${V_PERC}_${DATE}_new4060.apk" [gujian]="古建景点打卡V${V_GUJIAN}_${DATE}_new4060.apk" )
...
for f in "$OUT"/android/*.apk; do
  ...
  tmpapk="C:/tmp/_apksign_$$_$(basename "$f" | tr -cd 'A-Za-z0-9').apk"
  mkdir -p C:/tmp
  cp "$f" "$tmpapk"
  "$BT/apksigner.bat" verify "$tmpapk" >/dev/null 2>&1 && note "签名 OK $(basename "$f")" || fail "签名失败 $f"
  rm -f "$tmpapk"
done
...
cp "$ROOT"/native-shell/water-ios/apple-package/*_"${DATE}"_可托管_new4060.zip "$OUT/ios/" 2>/dev/null || fail "iOS zip 复制失败"
