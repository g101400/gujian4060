...
(function () {
  ...
  function officeOptions() {
    ...
    getCustomOffices().forEach(function (o) { if (o) set[o] = 1; });
    BUILDINGS.forEach(function (b) { var o = normOffice(b.office); if (o) set[o] = 1; });
    return Object.keys(set).sort();
  }
  // 周边/筛选/下拉共用的管理站选项（权威清单 + 自定义 + 数据实际出现）
  function stationOptions() {
    var set = {};
    getCustomStations().forEach(function (s) { if (s) set[s] = 1; });
    BUILDINGS.forEach(function (b) { var s = b.station || "（未设置）"; if (s) set[s] = 1; });
    return Object.keys(set).sort();
  }
  ...
  function buildMenu() {
    ...
    var groups = [
      ...
      { g: "传输与共享", ico: "🔗", items: [
        ...
        { ico: "📤", t: "导出 ovkmz（奥维）", f: openExportKmz },
        { ico: "📍", t: "导入 ovobj（奥维坐标）", f: importOvobj },
        { ico: "📤", t: "导出 ovobj（奥维坐标·实验）", f: openExportOvobj },
  ...
  function exportKmz(fileName, useFilter) {
  ...
  function openExportKmz() {
    ...
    var scopeHtml = filterActive()
      ...
      : "";
    var fileBase = APPNAME + "_" + getTodayStr();
    var html = scopeHtml +
      '<p style="font-size:13px;color:#555;margin-bottom:8px">导出奥维 kmz（含照片，可能较大）：</p>' +
      '<label class="f" style="font-size:13px;color:#555">导出文件名（不含扩展名）</label>' +
      '<input class="f" id="kmzFileName" value="' + esc(fileBase) + '">' +
  ...
  window.doExportKmz = function () {
    var rawName = ($("kmzFileName") && $("kmzFileName").value.trim()) || (APPNAME + "_" + getTodayStr());
    var fileName = rawName.replace(/[\\/:*?"<>|]/g, "_");
    var useFilter = $("kmzUseFilter") ? $("kmzUseFilter").checked : filterActive();
    exportKmz(fileName, useFilter);
  };
  // #353：ovobj 导出前对话框（文件名可改）
  function openExportOvobj() {
    ...
    var html =
      ...
      '<label class="f" style="font-size:13px;color:#555">导出文件名（不含扩展名）</label>' +
      '<input class="f" id="ovobjFileName" value="' + esc(fileBase) + '">' +
      '<p style="font-size:12px;color:#999;margin-top:6px">将保存至：Download/水利工程一张图/（安卓）或浏览器下载目录</p>' +
  ...
  window.doExportOvobj = function () {
    var rawName = ($("ovobjFileName") && $("ovobjFileName").value.trim()) || (APPNAME + "_" + getTodayStr());
    var fileName = rawName.replace(/[\\/:*?"<>|]/g, "_");
  ...
  function openExportPhotos() {
    ...
    var bldHtml = '<label class="f" style="font-size:13px;color:#555">🎯 按建筑物自动匹配管理所（可选）</label>' +
      ...
      '</select>';
    // #353：文件名可编辑（不含扩展名）
    var fileHtml = '<label class="f" style="font-size:13px;color:#555">导出文件名（不含扩展名）</label>' +
      '<input class="f" id="expPhotoName" value="' + esc(APPNAME + "_照片_" + getTodayStr()) + '">';
    var html =
  ...
  window.doExportPhotos = function () {
    ...
    BUILDINGS.forEach(function (b) {
      ...
      var no = normOffice(b.office) || "未设置管理所";   // 统一归一化比较，避免"温泉管理所"≠"温泉所"
    ...
    // #353：文件名可编辑（不含扩展名，默认按范围）
    var dflt = (office === "__ALL__" ? (APPNAME + "_照片_" + getTodayStr()) : (office + "_照片_" + getTodayStr()));
    var rawName = ($("expPhotoName") && $("expPhotoName").value.trim()) || dflt;
    var baseName = rawName.replace(/[\\/:*?"<>|]/g, "_");
  ...
  function exportTable() {
    ...
    // #353：文件名可编辑（不含扩展名）
    var fileBase = APPNAME + "_建筑物" + getTodayStr();
    var fileHtml = '<p style="font-size:13px;color:#555;margin:12px 0 6px">导出文件名（不含扩展名）</p>' +
      '<input class="f" id="expFileName" value="' + esc(fileBase) + '">';
  ...
  window.doExportTable = function () {
    ...
    // #353：文件名可编辑（非法字符清洗，默认回退建议名）
    var rawName = ($("expFileName") && $("expFileName").value.trim()) || (APPNAME + "_建筑物" + getTodayStr());
    var fileName = rawName.replace(/[\\/:*?"<>|]/g, "_") + ".csv";
