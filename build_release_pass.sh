...
for f in "$OUT"/android/*.apk; do
  ...
  # 2026-09-11 修复：apksigner.bat 是 Java 包装脚本，经 Git Bash 传递含中文的路径会被
  # 破坏（报 FileNotFoundException / 假失败）。aapt2.exe 能正确处理中文，apksigner 不能。
  # 对策：先复制到纯 ASCII 临时名再校验，保证「签名失败」只会是真失败。
