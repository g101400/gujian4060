...
(function () {
  ...
  window.onerror = function (msg, src, line, col) {
    ...
    if (t) { t.textContent = "运行错误：" + msg + " @" + line + ":" + col; t.classList.add("show"); }
    return false;
  };
  // 天地图 Token（请根据使用环境选择）
  // 浏览器端/WebView 内加载瓦片用此 token
  var TOKEN_BROWSER = "61691764ff68bf341c4c9c4770b24b5f";
  // 服务器端脚本拉取瓦片用此 token（本应用为纯前端，不直接使用，仅备查）
  var TOKEN_SERVER = "d258e3622cb1755b59a59e4b7ac6d96e";
  // 本应用在 WebView 中运行，使用浏览器端 token
  var TOKEN = TOKEN_BROWSER;
  var AUTHOR = "科技推广中心";
  var LS_KEY = "perception_map_v1";
  var APPNAME = "水利感知项目一张图";
  var APP_VERSION = "v1.6";
  var APP_BUILD_DATE = "2026-08-23";
  ...
  function loadSettings() {
    try { return Object.assign({ startPos: null }, JSON.parse(localStorage.getItem("perc_settings") || "{}")); }
    catch (e) { return { startPos: null }; }
  }
  function saveSettings() { try { localStorage.setItem("perc_settings", JSON.stringify(SETTINGS)); } catch (e) {} }
  // 运行维护计划（F3）：type ∈ 日常检查/例行维护/故障处理/应急响应/其他
  var MAINT_PLANS = loadMaintPlans();
  function loadMaintPlans() {
    try { var a = JSON.parse(localStorage.getItem("perc_maint_plans")); return Array.isArray(a) ? a : []; } catch (e) { return []; }
  }
  function saveMaintPlans() { try { localStorage.setItem("perc_maint_plans", JSON.stringify(MAINT_PLANS)); } catch (e) {} }

  var BUILDINGS = [];
  var MARKERS = {};
  var LINES = {};
  var filterLayer = null; // 筛选结果高亮层（倒水滴 + 绿色虚线圈）
  var map, baseVec, baseImg, labelVec, labelImg, baseOffline;
  var currentBase = "vec"; // "vec" | "img" | "offline"
  var OFFLINE_READY = false; // 离线底图包是否可用
  var addMode = false;
  var pendingAdd = null;
  var editing = null;
  var filters = { offices: new Set(), stations: new Set(), types: new Set(), subsystems: new Set(), midTypes: new Set(), subTypes: new Set(), ctrl: new Set(), transmit: new Set(), coordStatus: "all", photoStatus: "all", photoMin: 3, text: "" };
  ...
  function updateXferProgress(phase, done, total) {
    ...
    var label = { copy: "复制照片", zip: "打包", unzip: "解压", download: "下载",
                  netdisk: "上传网盘", "import": "解压导入", match: "智能匹配" }[phase] || phase;
  ...
  // 感知项目：按子系统着色（btype 承载子系统值）
  var CANONICAL_TYPES = [
    "视频监控", "雨水情监测", "大坝安全监测", "地下水源监测"
  ];
  // 即使当前数据尚未有该类，也始终在图例/筛选中展示
  var ALWAYS_TYPES = { "视频监控": true, "雨水情监测": true, "大坝安全监测": true, "地下水源监测": true };
  // 感知子系统（一期渲染主维度；颜色按子系统，形状仍按 btype）
  var SUBSYSTEMS = ["视频监控", "雨水情监测", "大坝安全监测", "地下水源监测"];
  var SUBSYSTEM_COLOR = {
    "视频监控": "#16324f",
    "雨水情监测": "#2e7d5b",
    "大坝安全监测": "#b8862f",
  ...
  function colorForSubsystem(s) { return SUBSYSTEM_COLOR[s] || "#888"; }
  function canonicalTypes() {
    var present = new Set();
    BUILDINGS.forEach(function (b) { if (b.btype) present.add(b.btype); });
    var out = CANONICAL_TYPES.filter(function (t) { return present.has(t) || ALWAYS_TYPES[t]; });
  ...
  /* ---------- 存储 ---------- */
  // 二期：从 attrs[[k,v]...] 抽取中类/小类/设备编码/可控性/传输方式/设备类型 为一级字段（旧 attrs 保留）
  var PERC_ATTR_KEYS = { "中类": "midType", "小类": "subType", "设备编码": "deviceCode", "是否可控": "ctrl", "信息传输方式": "transmit", "设备类型": "deviceType" };
  function normalizeRecord(b) {
    if (!b) return b;
    (b.attrs || []).forEach(function (kv) {
      if (!kv || kv.length < 2) return;
      var f = PERC_ATTR_KEYS[kv[0]];
      if (f && (b[f] == null || b[f] === "")) b[f] = kv[1];
    });
    return b;
  }
  function load() {
    var embedded = (window.PERCEPTION_DATA || []).map(normalizeRecord);
    try {
      var s = localStorage.getItem(LS_KEY);
      if (s) { BUILDINGS = JSON.parse(s).map(normalizeRecord); return; }
    } catch (e) {}
  ...
  function resetData() {
    ask("恢复初始数据", "确定恢复为初始数据？<b>所有新增/修改/删除将被清空</b>，且不可撤销。",
      ...
      function (ok) {
        if (!ok) return;
        BUILDINGS = JSON.parse(JSON.stringify(window.PERCEPTION_DATA || [])).map(normalizeRecord);
  ...
  function initMap() {
    ...
    labelImg = L.tileLayer("https://t{s}.tianditu.gov.cn/DataServer?T=cia_w&x={x}&y={y}&l={z}&tk=" + TOKEN,
      { subdomains: sub, maxZoom: 18, pane: "shadowPane" });
    // 四期：离线底图（本地瓦片，无需联网）。瓦片目录约定：
    //   assets/offline_tiles/{z}/{x}/{y}.png（打包进 APK 的小范围包）
    //   或手机目录 /Download/水利工程一张图/offline_tiles/...（大包，由原生桥映射）
    // 优先用原生桥提供的本地根路径（window.Android.tileBase()），否则退回 assets 相对路径。
    var offlineRoot = (window.Android && typeof window.Android.tileBase === "function")
      ? window.Android.tileBase() : "offline_tiles";
    baseOffline = L.tileLayer(offlineRoot + "/{z}/{x}/{y}.png",
      { maxZoom: 18, attribution: "离线底图（怀柔）", errorTileUrl: "", crossOrigin: false });
    // v3.30：已取消内置离线瓦片包（APK 体积优化），统一走在线天地图瓦片。离线选项停用。
    OFFLINE_READY = false;
    baseVec.addTo(map);
    labelVec.addTo(map);
    ...
    map.on("click", function (e) {
      ...
      openEdit({
        id: "b" + Date.now(),
        name: "新感知设施设备", office: "", station: "", chan: "", btype: "",
        subsystem: "", model: "", assetNo: "", params: [],
  ...
  function buildBaseSwitcher() {
    ...
    el.innerHTML =
      ...
      '<label class="bs-opt" style="display:none"><input type="radio" name="baseOpt" value="offline" disabled><span>离线地图</span></label>';
    el.querySelectorAll('input[name="baseOpt"]').forEach(function (radio) {
      radio.onchange = function () { setBase(this.value); };
    });
  }
  function setBase(mode) {
    if (mode === "offline" && !OFFLINE_READY) { toast("离线底图包未安装，已回退矢量地图"); mode = "vec"; }
    currentBase = mode;
    // 先移除全部底图与注记层，避免叠加
    [baseVec, labelVec, baseImg, labelImg, baseOffline].forEach(function (l) { if (l && map.hasLayer(l)) map.removeLayer(l); });
    if (mode === "vec") {
      map.addLayer(baseVec); map.addLayer(labelVec);
    } else if (mode === "img") {
      map.addLayer(baseImg); map.addLayer(labelImg);
    } else { // offline
      map.addLayer(baseOffline);
      // 试探首张瓦片可达性：命中则记忆为可用，未命中回退矢量
      baseOffline.once("tileload", function () {
        try { localStorage.setItem("perc_offline_ok", "1"); } catch (e) {}
      });
      baseOffline.once("tileerror", function () {
        try { localStorage.removeItem("perc_offline_ok"); } catch (e) {}
        OFFLINE_READY = false;
        toast("离线瓦片缺失，已回退矢量地图");
        if (currentBase === "offline") setBase("vec");
      });
    }
    // 与悬浮面板单选框状态保持同步
    var r = document.querySelector('#baseSwitcher input[value="' + currentBase + '"]');
    if (r) r.checked = true;
  }
  function switchBaseMap() {
    var order = ["vec", "img"];
    var idx = order.indexOf(currentBase);
    setBase(order[(idx + 1) % order.length]);
    toast("底图已切换为：" + (currentBase === "vec" ? "矢量地图" : "影像地图"));
  }
  // 四期：检测离线瓦片包是否可用
  //   1) 原生桥直接告知（Android.tileReady() 返回 true 表示手机目录已存在瓦片）
  //   2) 否则检测 assets/offline_manifest.json（打包进 APK 的小范围离线包）
  // 同步返回（初始化时用），异步细节由调用方兜底
  function detectOfflineTiles() {
    try {
      if (window.Android && typeof window.Android.tileReady === "function" && window.Android.tileReady()) return true;
    } catch (e) {}
    try {
      // 同步无法 fetch，这里仅标记“可能存在”，真正挂载时若 404 会自动降级（errorTileUrl 已置空）
      // 用 localStorage 记忆用户上次成功加载离线包的标记，避免每次都试错
      if (localStorage.getItem("perc_offline_ok") === "1") return true;
    } catch (e) {}
    return false;
  }
  ...
  function passFilter(b) {
    ...
    if (filters.subsystems.size && !filters.subsystems.has(b.subsystem || "")) return false;
    if (filters.midTypes.size && !filters.midTypes.has(b.midType || "")) return false;
    if (filters.subTypes.size && !filters.subTypes.has(b.subType || "")) return false;
    if (filters.ctrl.size && !filters.ctrl.has(b.ctrl || "")) return false;
    if (filters.transmit.size && !filters.transmit.has(b.transmit || "")) return false;
    // 三期：坐标状态筛选（待补坐标 / 已有坐标）
    var hasCoord = (typeof b.lat === "number" && typeof b.lon === "number");
    if (filters.coordStatus === "none" && hasCoord) return false;
    if (filters.coordStatus === "has" && !hasCoord) return false;
  ...
  function filterActive() {
    return !!(filters.offices.size || filters.stations.size || filters.types.size || filters.subsystems.size ||
      filters.midTypes.size || filters.subTypes.size || filters.ctrl.size || filters.transmit.size ||
      (filters.photoStatus && filters.photoStatus !== "all") || (filters.coordStatus && filters.coordStatus !== "all") || filters.text);
  }
  // 当前筛选命中的建筑列表
  ...
  function filterDesc() {
    ...
    if (filters.coordStatus === "none") parts.push("待补坐标");
    else if (filters.coordStatus === "has") parts.push("已有坐标");
    return parts.join("；");
  }
  ...
  function popupHtml(b) {
    ...
    if (b.btype) html += '<span class="badge" style="background:#16324f">' + esc(b.btype) + "</span>";
    if (b.office) html += '<span class="badge">' + esc(b.office) + "</span>";
    if (b.station) html += '<span class="badge">' + esc(b.station) + "</span>";
    if (b.noCoord) html += '<span class="badge" style="background:#c0392b">待补坐标</span>';
    if (b.subsystem) html += '<span class="badge" style="background:#1e6fbf">' + esc(b.subsystem) + "</span>";
    if (b.model) html += '<span class="badge" style="background:#0a7d6b">型号 ' + esc(b.model) + "</span>";
    if (b.assetNo) html += '<span class="badge" style="background:#9c4a3c">资产号 ' + esc(b.assetNo) + "</span>";
    if (b.params && b.params.length) {
      html += '<div class="eq-params">';
      b.params.forEach(function (p) { html += '<span class="eq-p"><i>参数</i>' + esc(p) + "</span>"; });
      html += "</div>";
    }
    // 二期：设备参数结构化块（中类/小类/可控性/传输方式/设备类型/设备编码）
    var eq = [];
    if (b.midType) eq.push(["中类", b.midType]);
    if (b.subType) eq.push(["小类", b.subType]);
    if (b.deviceType) eq.push(["设备类型", b.deviceType]);
    if (b.ctrl) eq.push(["是否可控", b.ctrl]);
    if (b.transmit) eq.push(["信息传输方式", b.transmit]);
    if (b.deviceCode) eq.push(["设备编码", b.deviceCode]);
    if (eq.length) {
      html += '<div class="eq-params">';
      eq.forEach(function (p) { html += '<span class="eq-p"><i>' + esc(p[0]) + '</i>' + esc(p[1]) + "</span>"; });
      html += "</div>";
    }
    ...
    html += '<div class="pop-actions"><button class="pop-edit" onclick="appEdit(\'' + esc(b.id) + '\')">修改</button>' +
  ...
  function render() {
    ...
    BUILDINGS.forEach(function (b) {
      ...
      if (b.geom === "Point" && b.lat != null) {
        ...
        var m = L.marker([b.lat, b.lon], { icon: makeIcon(colorForSubsystem(b.subsystem || "其他"), shapeForType(b.btype || "其他")) })
          .bindPopup(function () { return popupHtml(b); });
        m.addTo(map); MARKERS[b.id] = m; shown++;
      } else if (b.geom === "Line" && b.line && b.line.length) {
  ...
  window.openMaintPlanSheet = function () {
    var html = '<label class="f">计划类型</label><select class="f" id="mpType">' +
      ...
      uniq(BUILDINGS.map(function (b) { return normOffice(b.office); })).map(function (o) { return '<option>' + esc(o) + "</option>"; }).join("") + "</select>" +
  ...
  function openInspect(id) {
    ...
    $("ciPhoto").onchange = function () {
      ...
      r.onload = function () {
        ...
        try { if (window.Android && typeof window.Android.storePhoto === "function") editingInspect.photo = window.Android.storePhoto((id || "inspect_" + Date.now()), file.name, r.result); } catch (e) {}
        if (!editingInspect.photo) editingInspect.photo = r.result;
  ...
  function buildLegend() {
    // 一期：按感知子系统着色（主维度）
    var subCnt = {};
    BUILDINGS.forEach(function (b) { var s = b.subsystem || "其他"; subCnt[s] = (subCnt[s] || 0) + 1; });
    var html = "<b>感知子系统</b>";
    SUBSYSTEMS.forEach(function (s) {
      html += '<div class="lg"><span class="dot" style="background:' + colorForSubsystem(s) + '"></span>' + esc(s) + '　<span style="color:#888">' + (subCnt[s] || 0) + '</span></div>';
    });
    // 类型图例（次级，形状符号）
    var all = canonicalTypes();
    var types = all.slice(0, 8);
    html += "<b style='margin-top:6px;display:block'>建筑物类型</b>";
  ...
  /* ---------- 菜单 ---------- */
  function buildMenu() {
    // 一级菜单：查询 / 筛选 置顶并列；四大分组（二级默认隐藏，点标题或 ＋ 展开）
    var topItems = [
      { ico: "🔍", t: "查询（名称 / 类型 / 备注）", f: function () { closeSheet("sheetMenu"); setTimeout(function () { var s = $("search"); if (s) { s.focus(); s.select(); } }, 200); } },
      { ico: "🎚️", t: "筛选（管理所 / 管理站 / 类型）", f: openFilter }
    ];
    var groups = [
      { g: "运行维护", ico: "🛠️", items: [
        { ico: "📍", t: "默认出发位置", f: function () { closeSheet("sheetMenu"); openStartPosSheet(); } },
        { ico: "🗓️", t: "运行维护计划", f: function () { closeSheet("sheetMenu"); openMaintPlanSheet(); } },
        { ico: "📋", t: "运行维护计划列表", f: function () { closeSheet("sheetMenu"); openMaintPlanList(); } },
        { ico: "📷", t: "巡视（添加照片/巡查时间/情况）", f: function () { closeSheet("sheetMenu"); openInspect(null); } },
        { ico: "🛤️", t: "我的巡视路线", f: function () { closeSheet("sheetMenu"); openInspectRoute(); } }
      ]},
      { g: "地图和位置", ico: "🗺️", items: [
        ...
        { ico: "➕", t: "添加感知设施设备（点地图定位）", f: toggleAdd }
      ]},
      { g: "数据管理", ico: "🗂️", items: [
        { ico: "📊", t: "批量导入建筑物", f: batchImportBuildings },
        { ico: "🖼️", t: "批量导入照片", f: batchImportPhotos },
        { ico: "📑", t: "导出建筑物表格", f: exportTable },
        { ico: "🗂️", t: "导出照片（按管理所）", f: exportPhotos },
        { ico: "🧹", t: "清理导入缓存（释放空间）", f: cleanImportCache },
        { ico: "🗑️", t: "删除添加的照片", f: deleteImportedPhotos },
        { ico: "🏚️", t: "删除建筑物", f: deleteBuildingsMenu },
        { ico: "♻️", t: "恢复初始数据", f: resetData }
      ]},
      { g: "传输与共享", ico: "🔗", items: [
        { ico: "📥", t: "导入 ovkmz（奥维）", f: importKmz },
        { ico: "📤", t: "导出 ovkmz（奥维）", f: exportKmz },
        { ico: "🗂️", t: "按子系统导出 ovkmz", f: window.exportKmzBySub },
        { ico: "⚙️", t: "传输设置（WiFi/网盘/续传）", f: openXferSettings },
        { ico: "📡", t: "手机互传（服务端/客户端）", f: openPeer },
        { ico: "📲", t: "导出照片到统信平台", f: copyPhotosFromAndroid },
        { ico: "📦", t: "从安卓平台导出照片", f: exportPhotosForAndroid }
      ]},
      { g: "信息与帮助", ico: "ℹ️", items: [
  ...
  /* ---------- 筛选 ---------- */
  function openFilter() {
    // v3.26：管理所列表按统一名称去重（温泉管理所 与 温泉所 合并为一个筛选项）
    var offices = uniq(BUILDINGS.map(function (b) { return normOffice(b.office); }));
    var stations = uniq(BUILDINGS.map(function (b) { return b.station || "（未设置）"; }));
    ...
    function photoChips() {
      ...
      return opts.map(function (o) {
        ...
        if (o[0] === "many") {
          chip += ' <span class="photo-min-wrap" style="' + (filters.photoStatus === "many" ? "" : "display:none") + '">≥<input id="photoMinInput" type="number" min="1" step="1" value="' + filters.photoMin + '" style="width:46px;font-size:12px;padding:2px 3px;border:1px solid var(--border);border-radius:4px"></span>';
        }
        return chip;
      }).join("");
    }
    ...
    // 三期：坐标状态筛选（互斥单选：全部 / 待补坐标 / 已有坐标）
    function coordChips() {
      var opts = [["all", "全部"], ["none", "待补坐标"], ["has", "已有坐标"]];
      return opts.map(function (o) {
        return '<span class="chip coordchip' + (filters.coordStatus === o[0] ? " on" : "") + '" data-k="coordStatus" data-v="' + o[0] + '">' + o[1] + "</span>";
      }).join("");
    }
    // 感知一期：子系统筛选 chips（颜色点提示）
    function subsysChips() {
      return SUBSYSTEMS.map(function (s) {
        return '<span class="chip' + (filters.subsystems.has(s) ? " on" : "") + '" data-k="subsystem" data-v="' + esc(s) + '">' +
          '<span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:' + colorForSubsystem(s) + ';margin-right:5px;vertical-align:middle"></span>' + esc(s) + "</span>";
      }).join("");
    }
    ...
    function updateFilterCount() {
      ...
      if (filters.subsystems.size) {
        Array.from(filters.subsystems).forEach(function (v) {
          tags.push('<span class="fc-tag"><span class="fc-k" style="color:' + colorForSubsystem(v) + '">子系统</span>' + esc(v || "其他") + "</span>");
        });
      }
      if (filters.midTypes.size) {
        Array.from(filters.midTypes).forEach(function (v) {
          tags.push('<span class="fc-tag"><span class="fc-k">中类</span>' + esc(v || "未设置") + "</span>");
        });
      }
      if (filters.subTypes.size) {
        Array.from(filters.subTypes).forEach(function (v) {
          tags.push('<span class="fc-tag"><span class="fc-k">小类</span>' + esc(v || "未设置") + "</span>");
        });
      }
      if (filters.ctrl.size) {
        Array.from(filters.ctrl).forEach(function (v) {
          tags.push('<span class="fc-tag"><span class="fc-k">可控</span>' + esc(v || "未设置") + "</span>");
        });
      }
      if (filters.transmit.size) {
        Array.from(filters.transmit).forEach(function (v) {
          tags.push('<span class="fc-tag"><span class="fc-k">传输</span>' + esc(v || "未设置") + "</span>");
        });
      }
      ...
      if (filters.coordStatus === "none") tags.push('<span class="fc-tag"><span class="fc-k">坐标</span>待补坐标</span>');
      else if (filters.coordStatus === "has") tags.push('<span class="fc-tag"><span class="fc-k">坐标</span>已有坐标</span>');
    ...
    $("genBody").innerHTML =
      ...
      '<div class="filter-sec"><h4>管理所</h4><div class="chips" id="cOff">' + chips(offices, filters.offices, "office") + "</div></div>" +
      '<div class="filter-sec"><h4>管理站</h4><div class="chips" id="cSta">' + chips(stations, filters.stations, "station") + "</div></div>" +
      '<div class="filter-sec"><h4>建筑物类型</h4><div class="chips" id="cTyp">' + chips(types, filters.types, "type") + "</div></div>" +
      '<div class="filter-sec"><h4>感知子系统</h4><div class="chips" id="cSub">' + subsysChips() + "</div></div>" +
      '<div class="filter-sec"><h4>中类</h4><div class="chips" id="cMid">' + chips(uniq(BUILDINGS.map(function (b) { return b.midType || "（未设置）"; })), filters.midTypes, "midType") + "</div></div>" +
      '<div class="filter-sec"><h4>小类</h4><div class="chips" id="cSub2">' + chips(uniq(BUILDINGS.map(function (b) { return b.subType || "（未设置）"; })), filters.subTypes, "subType") + "</div></div>" +
      '<div class="filter-sec"><h4>是否可控</h4><div class="chips" id="cCtrl">' + chips(uniq(BUILDINGS.map(function (b) { return b.ctrl || "（未设置）"; })), filters.ctrl, "ctrl") + "</div></div>" +
      '<div class="filter-sec"><h4>信息传输方式</h4><div class="chips" id="cTx">' + chips(uniq(BUILDINGS.map(function (b) { return b.transmit || "（未设置）"; })), filters.transmit, "transmit") + "</div></div>" +
      '<div class="filter-sec"><h4>照片状态</h4><div class="chips" id="cPh">' + photoChips() + "</div></div>" +
      '<div class="filter-sec"><h4>坐标状态</h4><div class="chips" id="cCo">' + coordChips() + "</div></div>" +
    ...
    $("genBody").querySelectorAll(".chip").forEach(function (el) {
      el.onclick = function () {
        if (el.classList.contains("photochip")) {
          ...
          // 重新渲染照片状态区，显示/隐藏「≥N」输入框
          var cph = $("cPh"); if (cph) cph.innerHTML = photoChips();
          bindPhotoMin();
          render(); updateFilterCount();
          return;
        }
        if (el.classList.contains("coordchip")) {
          filters.coordStatus = el.dataset.v;
          $("genBody").querySelectorAll(".coordchip").forEach(function (c) { c.classList.toggle("on", c.dataset.v === filters.coordStatus); });
          render(); updateFilterCount();
          return;
        }
        var set = el.dataset.k === "office" ? filters.offices : el.dataset.k === "station" ? filters.stations : el.dataset.k === "type" ? filters.types : el.dataset.k === "subsystem" ? filters.subsystems : el.dataset.k === "midType" ? filters.midTypes : el.dataset.k === "subType" ? filters.subTypes : el.dataset.k === "ctrl" ? filters.ctrl : filters.transmit;
        var v = el.dataset.v;
  ...
  window.appClearFilter = function () {
    filters.offices.clear(); filters.stations.clear(); filters.types.clear(); filters.subsystems.clear();
    filters.midTypes.clear(); filters.subTypes.clear(); filters.ctrl.clear(); filters.transmit.clear();
    filters.photoStatus = "all";
    filters.coordStatus = "all";
    filters.text = "";
    var ft = $("filterText"); if (ft) ft.value = "";
    var sb = $("search"); if (sb) sb.value = "";
  ...
  function showFilterResult(matched, color) {
    ...
    matched.forEach(function (b) {
      ...
      if (b.geom === "Line" && b.line && b.line.length) {
        ...
        lat = mid[1]; lon = mid[0];
        var hl = L.marker([lat, lon], { icon: makeFilterIcon() }).bindPopup(popupHtml(b));
        markDrop(hl);
      } else if (b.lat != null && b.lon != null) {
        lat = b.lat; lon = b.lon; pts.push([lat, lon]);
        var m = L.marker([lat, lon], { icon: makeFilterIcon() }).bindPopup(popupHtml(b));
        markDrop(m);
  ...
  function openEdit(b) {
    editing = b;
    var offices = uniq(BUILDINGS.map(function (x) { return x.office; }));
    ...
    $("editTitle").textContent = b._new ? "新增感知设施设备" : "修改：" + b.name;
    $("editBody").innerHTML =
      '<label class="f">名称</label><input class="f" id="fName" value="' + esc(b.name) + '">' +
      '<label class="f">管理所</label><input class="f" id="fOffice" list="dlOffice" value="' + esc(b.office) + '"><datalist id="dlOffice">' + opt(offices, b.office) + "</datalist>" +
      '<label class="f">管理站</label><input class="f" id="fStation" value="' + esc(b.station) + '">' +
      ...
      '<label class="f">所属系统</label><input class="f" id="fSystem" list="dlSystem" value="' + esc(b.subsystem || "") + '"><datalist id="dlSystem">' + opt(SUBSYSTEMS, b.subsystem) + "</datalist>" +
      '<label class="f">设备型号</label><input class="f" id="fModel" value="' + esc(b.model || "") + '">' +
      '<label class="f">固定资产编号</label><input class="f" id="fAssetNo" value="' + esc(b.assetNo || "") + '">' +
      '<label class="f">设备主要参数（可多项，逐行增删）</label><div id="paramBox">' + (b.params || []).map(function (p, i) {
        return '<div class="kv-row"><input value="' + esc(p) + '" placeholder="如：量程 0-10MPa" data-pp="' + i + '"><button class="delkv" data-pi="' + i + '">×</button></div>';
      }).join("") + "</div>" +
      '<button class="tbtn" style="margin-top:6px" onclick="appAddParam()">+ 增加设备参数</button>' +
      ...
      ((typeof b.lat !== "number" || typeof b.lon !== "number") ? '<div style="color:#c0392b;font-size:12px;margin:-4px 0 8px">⚠ 该设施暂无精确坐标，填入经纬度后保存即可上图。</div>' : "") +
    ...
    $("editBody").querySelectorAll(".delkv").forEach(function (btn) {
      btn.onclick = function () {
        if (btn.dataset.di != null) { b.attrs.splice(+btn.dataset.di, 1); }
        else if (btn.dataset.pi != null) { (b.params = b.params || []).splice(+btn.dataset.pi, 1); }
        openEdit(b);
      };
    });
    $("fPhotos").onchange = function () {
      ...
      Array.prototype.forEach.call(files, function (file) {
        ...
        r.onload = function () {
          var rel = "";
          try { if (window.Android && typeof window.Android.storePhoto === "function") rel = window.Android.storePhoto(b.id, file.name, r.result); } catch (e) {}
          if (rel) b.photos.push({ file: rel, caption: "", added: Date.now() });
          else b.photos.push({ data: r.result, caption: "", added: Date.now() });
          openEdit(b);
        };
        r.readAsDataURL(file);
      });
  ...
  window.appAddKv = function () { editing.attrs.push(["", ""]); openEdit(editing); };
  window.appAddParam = function () { editing.params = editing.params || []; editing.params.push(""); openEdit(editing); };
  window.appSave = function () {
    ...
    b.subsystem = $("fSystem").value.trim();
    b.model = $("fModel").value.trim();
    b.assetNo = $("fAssetNo").value.trim();
    b.params = [];
    var pbox = $("paramBox");
    if (pbox) pbox.querySelectorAll("input[data-pp]").forEach(function (inp) {
      var v = inp.value.trim(); if (v) b.params.push(v);
    });
  ...
  function renderList() {
    ...
    rows.forEach(function (b) {
      var noCoord = !(typeof b.lat === "number" && typeof b.lon === "number");
      html += '<div class="list-card">' +
        '<span class="go" onclick="appFly(\'' + esc(b.id) + '\')">快速定位 ›</span>' +
        (noCoord ? '<span class="go rec" onclick="appRecCoord(\'' + esc(b.id) + '\')">补录坐标</span>' : "") +
        "<h3>" + esc(b.name) + "</h3>" +
        '<div class="meta">' + esc(b.office) + " · " + esc(b.btype) + (b.station ? " · " + esc(b.station) : "") + ((b.inspections || []).length ? " · 已巡视" + b.inspections.length : "") + (noCoord ? ' · <span style="color:#c0392b">待补坐标</span>' : "") + "</div></div>";
    });
    $("listView").innerHTML = html;
  }
  // 三期：一键补录坐标——关闭列表、飞到该建筑并打开编辑页（光标聚焦经纬度输入）
  window.appRecCoord = function (id) {
    var b = BUILDINGS.find(function (x) { return x.id === id; });
    if (!b) return;
    listMode = false; $("listView").style.display = "none"; $("map").style.display = "block";
    setTimeout(function () {
      map.invalidateSize();
      map.setView([40.30, 116.70], 11);
      openEdit(b);
      setTimeout(function () { var f = $("fLon"); if (f) f.focus(); }, 200);
    }, 150);
    toast("请在「经度/纬度」填入精确坐标后保存");
  };
  ...
  function nearbySearch() {
    ...
    var html = '<label class="f">选择中心建筑物</label>' +
      '<select class="f" id="nearbyCenter" style="margin-bottom:10px">' +
      pts.map(function (b) { return '<option value="' + esc(b.id) + '">' + esc(b.name) + '</option>'; }).join("") +
      '</select>' +
      '<label class="f">搜索半径（米）</label>' +
      '<input class="f" type="number" id="nearbyRadius" value="1000" min="50" max="50000" style="margin-bottom:10px">' +
  ...
  window.appDoNearby = function () {
    ...
    if (!centerId || isNaN(radius) || radius < 50) { toast("请选择建筑物并输入有效半径"); return; }
    var center = BUILDINGS.find(function (b) { return b.id === centerId; });
    if (!center || center.lat == null) { toast("中心建筑物无坐标"); return; }
    var centerLatLng = L.latLng(center.lat, center.lon);
    var results = [];
    BUILDINGS.forEach(function (b) {
      if (b.id === centerId || b.lat == null) return;
    ...
    if (results.length === 0) {
      html += '<p style="color:#888">范围内无其他建筑物</p>';
    } else {
  ...
  window.onPickFiles = function (json) {
    ...
    if (!list || !list.length) { toast("未选择文件"); window.__kmzImport = false; window.__bldImport = false; return; }
    ...
    window.__uzAcc = { items: items, total: archives.length, done: 0 };
  ...
  // 原生异步解压回调：累积结果，全部完成后再统一匹配
  window.onUnzipImages = function (json) {
    var arr; try { arr = JSON.parse(json); } catch (e) { arr = []; }
    if (typeof arr === "string") { toast((arr + "").replace(/^ERR:/, "")); }
    else { (arr || []).forEach(function (x) { window.__uzAcc.items.push({ fname: nameNoExt(x.name), base: normalizePhotoName(x.name), path: x.path, folder: x.folder || "" }); }); }
    window.__uzAcc.done++;
    finishUzIfDone();
  };
  ...
  // 修复：把文件夹「每一层目录名」和「文件名」都拿去跟建筑物名比对，取各建筑物最高分。
  function bestMatchesForItem(it) {
    ...
    keys.forEach(function (k) {
      smartFuzzyMatches(k, BUILDINGS).forEach(function (m) {
        var id = m.b.id;
  ...
  function linkOrBase64(b, it) {
    ...
    if (window.Android && window.Android.linkPhoto) {
      try {
        ...
        if (rel) { b.photos.push({ file: rel, caption: cap, added: Date.now() }); return true; }
      } catch (e) {}
    }
  ...
  function runPhotoMatchNative(items) {
    ...
    function step() {
      ...
      else { closeXferProgress(); finishPhotoMatch(r); }
    }
    step();
  }
  ...
  function finishPhotoMatch(r) {
    r = r || {};
    save(); render();
    // 注意：此处【不】立即 cleanInbox()。未匹配/多选/重复照片的源文件仍在 inbox/uz_*，
    // 必须等用户在确认 UI 中真正绑定（或取消）后再清理，否则手动绑定时 linkPhoto 打开已删除文件会静默失败。
    // 清理统一交给 maybeCleanInbox()，在手动匹配框全部处理完毕后才执行。
    maybeCleanInbox();
  ...
  function batchImportBuildings() {
    var html = '<div style="font-size:14px;color:#555;margin-bottom:12px">' +
      ...
      '<p style="color:#888;font-size:12px">支持列：name(名称), lat(纬度), lon(经度), office(管理所), station(感知对象), btype(子系统), chan(设备类型)。<br>有经纬度的自动定位；无经纬度的标记为「待补坐标」按管理所中心落点。</p></div>' +
  ...
  function readBuildingFile(f, fname) {
    ...
    setTimeout(function () {
      try {
        if (/\.csv$/i.test(fname)) {
          var text = window.Android.readFileText(f.path);
          idle();
  ...
  function processBuildingRows(rows) {
    ...
    var iStation = findCol(["station", "管理站", "感知对象"]);
    var iBtype = findCol(["btype", "type", "子系统", "建筑物类型", "类型", "感知设施大类"]);
    var iChan = findCol(["chan", "段", "设备类型", "中类"]);
    // 三期：台账增量导入扩展列（设备编码为主键，中类/小类/可控/传输/子系统）
    var iDeviceCode = findCol(["devicecode", "设备编码", "设施编码"]);
    var iSubsystem = findCol(["subsystem", "感知子系统", "子系统"]);
    var iMid = findCol(["midtype", "中类", "感知设施中类"]);
    var iSub = findCol(["subtype", "小类", "感知设施小类"]);
    var iCtrl = findCol(["ctrl", "是否可控", "可控性"]);
    var iTransmit = findCol(["transmit", "信息传输方式", "传输方式"]);

    if (iName < 0 && iDeviceCode < 0) { $("batchBldResult").innerHTML = "<span style='color:#c0392b'>未找到\"名称\"或\"设备编码\"列</span>"; return; }

    function cell(row, i) { return (i >= 0 && row[i] != null) ? String(row[i]).trim() : ""; }
    // 先解析成行对象（跳过空名称且无设备编码的行），并去重“文件内部同设备编码行”（同编码只保留最后一行）
    var parsed = [];
    for (var r = 1; r < rows.length; r++) {
      var row = rows[r];
      if (!row) continue;
      var nm = cell(row, iName), dc = cell(row, iDeviceCode);
      if (!nm && !dc) continue;
      parsed.push({
        name: nm,
        deviceCode: dc,
        ...
        station: cell(row, iStation),
        btype: cell(row, iBtype) || "其他",
        chan: cell(row, iChan),
        subsystem: cell(row, iSubsystem),
        midType: cell(row, iMid),
        subType: cell(row, iSub),
        ctrl: cell(row, iCtrl),
        transmit: cell(row, iTransmit),
        noCoord: (iLat < 0 || iLon < 0 || isNaN(parseFloat(row[iLat])) || isNaN(parseFloat(row[iLon])))
      });
    }
    if (!parsed.length) { $("batchBldResult").innerHTML = "<span style='color:#c0392b'>未解析到数据行</span>"; return; }
    // 三期：文件内部按 deviceCode 去重（有编码优先，同编码只留最后一行）；无编码则按名称去重
    var seen = {};
    parsed = parsed.filter(function (p) {
      var key = p.deviceCode ? "dc:" + p.deviceCode : "nm:" + p.name;
      if (seen[key]) return false; seen[key] = true; return true;
    });

    // 三期：主键匹配规则（对导入前快照判断，避免相互污染）
    //   1) deviceCode 非空 → 优先按设备编码精确匹配
    //   2) 否则按名称完全相同匹配
    //   3) 仍无 → 无坐标时按名称包含匹配坐标（沿用旧逻辑）
    var existCode = {}, existName = {};
    BUILDINGS.forEach(function (b) {
      if (b.deviceCode) existCode[b.deviceCode] = b;
      if (b.name) existName[b.name] = b;
    });
    var findExisting = function (p) {
      if (p.deviceCode && existCode[p.deviceCode]) return existCode[p.deviceCode];
      if (p.name && existName[p.name]) return existName[p.name];
      return null;
    };
    var dupNames = parsed.filter(function (p) { return existName[p.name] || (p.deviceCode && existCode[p.deviceCode]); })
      .map(function (p) { return p.deviceCode || p.name; })
      .filter(function (n, i, a) { return a.indexOf(n) === i; });

    // 把结构化字段回写到一级字段 + attrs（保留原有非结构化 attrs）
    function mergeAttrs(b, p) {
      var map = { "中类": p.midType, "小类": p.subType, "设备编码": p.deviceCode, "是否可控": p.ctrl, "信息传输方式": p.transmit };
      if (p.subsystem && SUBSYSTEMS.indexOf(p.subsystem) >= 0) b.subsystem = p.subsystem;
      b.attrs = b.attrs || [];
      Object.keys(map).forEach(function (k) {
        var v = map[k]; if (!v) return;
        var row = b.attrs.find(function (a) { return a[0] === k; });
        if (row) row[1] = v; else b.attrs.push([k, v]);
      });
      normalizeRecord(b);
    }
    ...
    function applyRows(list) {
      var added = 0, updated = 0, noCoord = 0, keptInsp = 0;
      var details = [];
      var seq = 0;
      list.forEach(function (p) {
        var existing = findExisting(p);
        if (existing) {
          // 合并（不覆盖已有巡视/照片）
          if (p.office) existing.office = p.office;
          if (p.station) existing.station = p.station;
          if (p.btype && p.btype !== "其他") existing.btype = p.btype;
          if (p.chan) existing.chan = p.chan;
          if (!isNaN(p.lat) && !isNaN(p.lon)) { existing.lat = p.lat; existing.lon = p.lon; existing.geom = "Point"; }
          mergeAttrs(existing, p);
          keptInsp += (existing.inspections || []).length;
          updated++;
          details.push("✎ 更新：" + (p.deviceCode ? p.deviceCode + " " : "") + (p.name || "(无名称)"));
        } else {
          var nb = { id: "imp" + Date.now() + (seq++), name: p.name, office: p.office, station: p.station, chan: p.chan, btype: p.btype, path: "", attrs: [], photos: [], geom: "Point", inspections: [] };
          if (p.deviceCode) nb.deviceCode = p.deviceCode;
          if (p.subsystem && SUBSYSTEMS.indexOf(p.subsystem) >= 0) nb.subsystem = p.subsystem;
          if (!isNaN(p.lat) && !isNaN(p.lon)) {
            nb.lat = p.lat; nb.lon = p.lon;
          } else {
            var match = BUILDINGS.find(function (b) { return b.name && p.name && b.name.indexOf(p.name) >= 0 && b.lat != null; });
          ...
          BUILDINGS.push(nb);
          added++;
          details.push("✚ 新增：" + (p.deviceCode ? p.deviceCode + " " : "") + (p.name || "(无名称)"));
        }
      });
      save(); render(); buildLegend();
      var html = '<p style="color:#2b8a5d;font-weight:600">新增：' + added + '　更新：' + updated + (noCoord ? "　无坐标：" + noCoord : "") + (keptInsp ? "　保留巡视：" + keptInsp + " 次" : "") + "</p>";
    ...
    if (!dupNames.length) { applyRows(parsed); return; }

    var shown = dupNames.slice(0, 5).map(esc).join("、");
    var more = dupNames.length > 5 ? " 等 " + dupNames.length + " 个" : "";
    ask("存在可合并记录",
      "导入的文件中有 <b>" + dupNames.length + "</b> 条与现有数据（按设备编码/名称）匹配（" + shown + more + "）。合并时将<b>保留已有巡视与照片</b>，仅更新台账字段。如何处理？",
      [
        { t: "合并更新", cls: "btn-confirm2", v: "overwrite" },
        { t: "仅加新记录", cls: "btn-cancel", v: "keep" },
        { t: "取消导入", cls: "btn-exit", v: "cancel" }
      ],
      function (mode) {
        if (mode === "cancel") { $("batchBldResult").innerHTML = "<p style='color:#888'>已取消导入，未作任何改动。</p>"; return; }
        applyRows(mode === "overwrite" ? parsed : parsed.filter(function (p) { return !findExisting(p); }));
      });
  }
  ...
  function showStats() {
    var byType = {}, byOffice = {}, bySub = {}, subCoord = {}, subInsp = {};
    var pts = 0, lns = 0, ph = 0, totalPhotos = 0, totalInspect = 0, inspBuildings = 0;
    BUILDINGS.forEach(function (b) {
      var st = b.btype || "其他";
      byType[st] = (byType[st] || 0) + 1;
      byOffice[b.office || "未设置"] = (byOffice[b.office || "未设置"] || 0) + 1;
      var sub = b.subsystem || "其他";
      bySub[sub] = (bySub[sub] || 0) + 1;
      var hasCoord = (typeof b.lat === "number" && typeof b.lon === "number");
      if (!subCoord[sub]) subCoord[sub] = { total: 0, ok: 0 };
      subCoord[sub].total++; if (hasCoord) subCoord[sub].ok++;
      var ni = (b.inspections || []).length;
      subInsp[sub] = (subInsp[sub] || 0) + ni;
    ...
    function subRows() {
      return SUBSYSTEMS.map(function (s) {
        var c = subCoord[s] || { total: 0, ok: 0 };
        var rate = c.total ? Math.round(c.ok / c.total * 100) : 0;
        return '<div class="sub-row"><span class="dot" style="background:' + colorForSubsystem(s) + '"></span>' +
          '<span class="sub-name">' + esc(s) + '</span>' +
          '<span class="sub-cnt">' + (bySub[s] || 0) + ' 个</span>' +
          '<span class="sub-rate">坐标 ' + rate + '%</span>' +
          '<span class="sub-insp">巡视 ' + (subInsp[s] || 0) + '</span></div>';
      }).join("");
    }
    ...
    $("genBody").innerHTML =
      ...
      "<p style='margin-top:14px'><b>按感知子系统</b></p><div class='sub-list'>" + subRows() + "</div>" +
  ...
  /* ---------- 帮助 / 关于 ---------- */
  function showHelp() {
    $("genTitle").textContent = "帮助";
    $("genBody").innerHTML =
      "<p><b>① 筛选</b>：按管理所 / 管理站 / 建筑物类型组合过滤，或顶部搜索框关键字检索。</p>" +
      "<p><b>② 添加</b>：菜单「添加建筑物」→ 点地图放置 → 填写信息、上传多张照片。</p>" +
      "<p><b>③ 修改</b>：点击标记弹出 → 「修改」编辑名称/坐标/参数/照片。</p>" +
      "<p><b>④ 删除</b>：弹出或编辑页「删除」。</p>" +
      "<p><b>⑤ 导航</b>：点击建筑物标记 → 弹窗中点「导航」，选择手机已安装的导航App。</p>" +
      "<p><b>⑥ 测距</b>：菜单「测距」→ 依次点击地图上的点，自动显示各段距离和总距离。双击或再点「结束测距」结束。</p>" +
      "<p><b>⑦ 周边搜索</b>：菜单「周边搜索」→ 选择中心建筑物和半径，查找范围内的所有建筑物。</p>" +
      "<p><b>⑧ 获取坐标</b>：菜单「获取坐标」→ 支持GPS定位、点选地图、搜索建筑物三种方式获取经纬度，可一键复制。</p>" +
      "<p><b>⑨ 批量导入照片</b>：菜单「批量导入照片」→ 选择多张照片或ZIP压缩包，按文件名自动匹配建筑物。</p>" +
      "<p><b>⑩ 批量导入建筑物（台账更新）</b>：菜单「批量导入建筑物」→ 选择CSV/Excel/ZIP文件，按列名解析；优先按「设备编码」去重合并（缺编码则按名称+坐标匹配），<b>保留已有巡视与照片</b>仅更新台账字段；新列支持中类/小类/可控/传输/子系统。</p>" +
      "<p><b>⑪ 定位</b>：菜单「定位我的位置」，如未授权自动跳转设置。</p>" +
      "<p><b>⑫ 导入/导出 ovkmz</b>：兼容奥维互动地图格式，可回灌；传输与共享新增「按子系统导出 ovkmz」（每个子系统一个包，打包 ZIP）。</p>" +
      "<p><b>⑬ 导出照片</b>：菜单「导出照片（按管理所）」→ 按管理所分文件夹打包为 ZIP，多张照片自动加序号。</p>" +
      "<p><b>⑭ 导出建筑物表格</b>：菜单「导出建筑物表格」→ 勾选字段列、按管理所筛选，可勾选「按感知子系统分别导出」（每个子系统一个 CSV，打包 ZIP）。</p>" +
      "<p><b>⑮ 坐标补录</b>：筛选选「坐标状态=待补坐标」定位缺失设施；列表视图对无坐标项显示「补录坐标」一键入口，打开编辑页聚焦经纬度填写后保存即上图。</p>" +
      "<p><b>⑮ 运行维护</b>：菜单「运行维护」分组——<b>默认出发位置</b>（地图上选点或选建筑物，含默认出发时间，用于路线时长估算）；<b>运行维护计划</b>/<b>运行维护计划列表</b>（计划类型、起止日期、处理时间、范围、备注）；<b>巡视</b>选建筑物、拍巡查照片、填巡视类型/巡查时间/坐标（自动获取）/是否计划内/时长/巡查情况，自动记入「我的巡视路线」（含按天外勤时长汇总，照片点开可放大循环查看）。</p>" +
      "<p style='color:#888;font-size:12px;margin-top:8px'>提示：三击顶部标题或点右下角悬浮按钮可快速打开菜单。</p>" +
      "<p><b>底图</b>：天地图矢量/影像，需联网；离线时标记仍在。另支持<b>离线地图</b>（怀柔离线瓦片包）：将离线瓦片放入手机 <code>Download/水利工程一张图/offline_tiles/</code>（或随 APK 内置小范围包），底图切换器选「离线地图」即可无网查看范围。</p>" +
      "<p>数据保存在本机（localStorage），「恢复初始数据」可清空改动。</p>";
    openSheet("sheetGen");
  }
  function showAbout() {
    var m = window.PERCEPTION_META || {};
    var env = detectEnv();
    // 制作环境（开发机固定信息）
    var buildEnv = [
      ["操作系统", "Windows 10 Pro"],
  ...
  /* ---------- 版本变更 ---------- */
  var CHANGES = [
    { v: "v1.6", d: "2026-08-23", items: [
      "优化（APK 体积 · 需求①）：删除内置离线瓦片包（offline_tiles 2216 张 / offline_manifest.json），APK 由 11.5MB 降至约 300KB；底图统一走在线天地图瓦片（矢量/影像），离线地图选项停用。无网时地图区域空白，但全部业务功能（台账/巡视/筛选/导入导出）不受影响。",
      "优化（设施录入字段 · 需求②）：菜单「添加建筑物」更名为「添加感知设施设备（点地图定位）」；编辑页新增「所属系统」（下拉：视频监控/雨水情监测/大坝安全监测/地下水源监测）、「设备型号」、「固定资产编号」；新增「设备主要参数」可多行增删块（逐项记录量程/精度等）；弹窗同步展示上述字段。",
      "恢复（照片目录菜单 · 需求③）：信息与帮助分组重新挂回「照片（目录与命名）」入口，展示四端照片存放目录与命名约定（photos/<管理所>/<建筑物>/<序号>.jpg），便于复制拷贝核对。"
    ]},
    { v: "v1.5", d: "2026-08-22", items: [
      "四期（离线底图骨架）：底图切换器新增「离线地图」选项（与矢量/影像并列）。离线底图指向本地瓦片 offline_tiles/{z}/{x}/{y}.png——支持随 APK 内置小范围包或手机目录 Download/水利工程一张图/offline_tiles/（原生桥 tileBase/tileReady 映射）。无离线包时该选项禁用并回退；切换离线后若瓦片缺失自动降级矢量并记录状态。",
      "四期（瓦片下载工具）：新增 tools/download_tiles.py（Python），按怀柔 bbox + 缩放级抓取天地图瓦片生成本地目录与 offline_manifest.json，可 --dry 估算、--limit 调试、多线程下载。"
    ]},
    { v: "v1.4", d: "2026-08-22", items: [
      "三期（数据更新闭环）：台账增量导入支持按「设备编码」去重合并（缺失则名称+坐标二级匹配），合并时保留已有巡视与照片、仅更新台账字段；新增列映射（设备编码/中类/小类/可控/传输/子系统），抽取字段回写一级字段与 attrs。",
      "三期（坐标补录工作流）：筛选新增「坐标状态」（待补坐标 / 已有坐标）；列表视图对无坐标设施标记「待补坐标」并提供「补录坐标」一键入口（飞到设施并打开编辑页聚焦经纬度）；编辑页对无坐标设施显示补录提示。",
      "三期（按子系统分组导出）：导出建筑物表格新增「按感知子系统分别导出」（每个子系统一个 CSV，整体打包 ZIP，另附全部汇总）；传输与共享新增「按子系统导出 ovkmz」（每个子系统一个 ovkmz，打包 ZIP）。"
    ]},
    { v: "v1.5", d: "2026-08-23", items: [
      "修复（安装无桌面图标/不提示打开 · 根因）：AndroidManifest.xml 的 Launcher category 误写为 android.category.LAUNCHER（少了 intent.），系统无法识别启动入口。改为标准 android.intent.category.LAUNCHER，重打 APK 后安装正常出图标、可打开。",
      "优化（删除「照片（目录与命名）」说明页菜单项）：按用户要求移除该页入口（保留函数，不删核心功能）。内置照片已于 v3.28 移除，本页为四端照片目录约定说明，移除后不影响任何导入/显示功能。"
    ]},
    { v: "v1.3", d:  "2026-08-22", items: [
      "一期（子系统渲染）：标记按感知子系统着色（视频监控/雨水情/大坝安全/地下水源 四色），形状仍按建筑物类型；图例改为按子系统分组展示（4 色 + 各类计数）；筛选新增「感知子系统」维度；统计页新增「各子系统设施数 / 含坐标率」。",
      "二期（参数结构化 + 巡视联动）：运行期从 attrs 抽取「中类/小类/设备编码/是否可控/信息传输方式/设备类型」为一级字段（旧 attrs 保留）；筛选新增「中类/小类/是否可控/信息传输方式」四个维度；弹窗新增设备参数块；统计页各子系统显示巡视次数；导出表格新增上述字段可选列。",
      "数据校验：本版数据严格为 PERCEPTION_DATA（1125 条，全部带数字经纬度），已修正早期错用 SHUILI_DATA（557 条）打包的问题。"
    ]},
    { v: "v1.1", d: "2026-08-22", items: [
      "新增（运行维护 · 与水利一张图同源）：菜单「运行维护」分组，新增「默认出发位置」「运行维护计划」「运行维护计划列表」，并升级「巡视」含巡视类型/自动坐标/是否计划内，路线页含按天外勤时长汇总与单图放大循环查看。",
      "原「巡视检查」分组更名为「运行维护」（语义映射：古建「打卡」对应水利「巡视检查」——设施运行巡查、记录现场状态与处置）。",
  ...
  function mergeImported(list, spec, keptExisting) {
    ...
    setTimeout(function () {
      ...
      ask("导入完成", msg, [{ t: "知道了", cls: "btn-confirm2", v: 1 }], function () {});
  ...
  function parseKmlWithPhotos(xml, photos) {
    ...
    for (var pi = 0; pi < pms.length; pi++) {
      ...
      for (var k = 0; k < atta.length; k++) {
        ...
        if (ph.relPath) photosArr.push({ file: ph.relPath, caption: "" });
  ...
  /* ---------- 导出 ovkmz（原生大文件管线：KML 由 JS 组装，照片走磁盘 + 续传 + 进度）---------- */
  // 三期：按子系统分别导出 ovkmz（每个子系统一个包，整体打包 ZIP）
  window.exportKmzBySub = function () {
    if (!BUILDINGS.length) { toast("没有可导出的数据"); return; }
    var groups = SUBSYSTEMS.map(function (s) { return { s: s, list: BUILDINGS.filter(function (b) { return (b.subsystem || "其他") === s; }) }; })
      .filter(function (g) { return g.list.length; });
    if (!groups.length) { toast("暂无可导出的子系统数据"); return; }
    busy("正在按子系统导出 ovkmz（" + groups.length + " 组）…");
    setTimeout(function () {
      var zip = new JSZip(); var tasks = [];
      groups.forEach(function (g) {
        var spec; try { spec = buildExportSpec(g.list); } catch (e) { return; }
        var fn = (APPNAME + "_" + g.s + "_" + getTodayStr() + ".ovkmz").replace(/[\\/:*?"<>|]/g, "_");
        tasks.push(buildKmzBlob(spec).then(function (blob) { zip.file(fn, blob); }).catch(function () {}));
      });
      Promise.all(tasks).then(function () {
        return zip.generateAsync({ type: "blob", compression: "DEFLATE" });
      }).then(function (blob) {
        idle();
        saveBlobFile(blob, APPNAME + "_按子系统_ovkmz_" + getTodayStr() + ".zip");
        toast("已按子系统导出 " + groups.length + " 个 ovkmz（ZIP）");
      }).catch(function (e) { idle(); toast("导出失败：" + e.message); });
    }, 30);
  };
  function exportKmz() {
    ...
    pickExportScope("ovkmz", function (list, scope) {
      busy("正在整理导出数据（" + list.length + " 个建筑）…");
      // 让忙提示先渲染出来，再做可能较重的 KML 组装
      setTimeout(function () {
        var spec, fileName;
        try {
          spec = buildExportSpec(list);
          fileName = APPNAME + (scope === "筛选结果" ? "_筛选" : "") + "_" + getTodayStr() + ".ovkmz";
        ...
        if (window.Android && typeof window.Android.exportKmz === "function") {
          ...
          ensureWifi("导出 ovkmz（" + scope + "：" + list.length + " 个建筑，" + spec.photos.length + " 张照片）", function () {
            openXferProgress("export", "正在导出 " + fileName + " …");
  ...
  // 把 spec 拼成 ovkmz Blob（doc.kml + 照片），供整体导出与按子系统拆分共用
  function buildKmzBlob(spec) {
    var zip = new JSZip();
  ...
  function legacyExportKmz(list, scope) {
    busy("正在打包导出，请稍后…");
    var spec = buildExportSpec(list && list.length ? list : BUILDINGS);
    buildKmzBlob(spec).then(function (blob) {
      idle();
      var fileName = APPNAME + (scope === "筛选结果" ? "_筛选" : "") + "_" + getTodayStr() + ".ovkmz";
      if (window.Android && typeof window.Android.saveBlob === "function") {
        var r = new FileReader();
        r.onload = function () { window.Android.saveBlob(r.result, fileName); };
        r.readAsDataURL(blob);
        toast("正在导出到 Download/水利工程一张图/" + fileName);
        return;
      }
  ...
  function openExportPhotos() {
    var offices = uniq(BUILDINGS.filter(function (b) { return b.photos && b.photos.length; }).map(function (b) { return b.office || "未设置管理所"; }));
    if (!offices.length) { toast("没有可导出的照片"); return; }
    var officeOpts = '<label class="exp-radio"><input type="radio" name="expOffice" value="__ALL__" checked> 全部管理所（合并一个压缩包，按管理所分文件夹）</label>' +
      offices.map(function (o) { return '<label class="exp-radio"><input type="radio" name="expOffice" value="' + esc(o) + '"> ' + esc(o) + '（单独一个压缩包）</label>'; }).join("");
    var html =
      '<p style="font-size:13px;color:#555;margin-bottom:8px">选择照片导出方式：</p>' +
      '<div class="filter-sec"><h4>范围</h4><div class="exp-radios">' + officeOpts + '</div></div>' +
  ...
  window.doExportPhotos = function () {
    var office = (document.querySelector('input[name="expOffice"]:checked') || {}).value || "__ALL__";
    var fmt = (document.querySelector('input[name="expFmt"]:checked') || {}).value || "zip";
    var target = (document.querySelector('input[name="expTarget"]:checked') || {}).value || "local";
    var files = [];
    BUILDINGS.forEach(function (b) {
      if (!b.photos || !b.photos.length) return;
      if (office !== "__ALL__" && (b.office || "未设置管理所") !== office) return;
      var folder = (b.office || "未设置管理所");
      var bn = (b.name || "建筑物");
      b.photos.forEach(function (p, i) {
        if (!p.file) return;
    ...
    var baseName = (office === "__ALL__" ? (APPNAME + "_照片_" + getTodayStr()) : (office.replace(/[\\/:*?"<>|]/g, "_") + "_照片_" + getTodayStr()));
    runExportPhotos(files, baseName, target, fmt);
  };
  function runExportPhotos(files, baseName, target, fmt) {
    if (!(window.Android && typeof window.Android.exportPhotos === "function")) { legacyExportPhotos(); return; }
    var ext = "zip";
    if (fmt === "7z") {
  ...
  // 浏览器回退：JSZip 打包
  function legacyExportPhotos() {
    var offices = {};
    var total = 0;
    BUILDINGS.forEach(function (b) {
      if (!b.photos || !b.photos.length) return;
      var o = b.office || "未设置管理所";
    ...
    Promise.all(tasks).then(function () {
      ...
      idle(); saveBlobFile(blob, APPNAME + "_照片_" + getTodayStr() + ".zip");
      toast("照片已导出");
    }).catch(function (e) { idle(); toast("导出失败：" + e.message); });
  }
  ...
  function exportTable() {
    var baseCols = [
      { k: "name", t: "名称" }, { k: "office", t: "管理所" }, { k: "station", t: "管理站" },
      { k: "chan", t: "段" }, { k: "btype", t: "建筑物类型" }, { k: "subsystem", t: "子系统" },
      { k: "midType", t: "中类" }, { k: "subType", t: "小类" }, { k: "deviceType", t: "设备类型" },
      { k: "ctrl", t: "是否可控" }, { k: "transmit", t: "信息传输方式" }, { k: "deviceCode", t: "设备编码" },
      { k: "lat", t: "纬度" }, { k: "lon", t: "经度" }, { k: "params", t: "参数说明" }, { k: "inspect", t: "巡视次数" }
    ];
    ...
    var colHtml = baseCols.map(function (c) {
      return '<label style="display:flex;align-items:center;gap:6px;padding:4px 0;font-size:14px"><input type="checkbox" class="expCol" value="' + c.k + '" checked> ' + c.t + "</label>";
    }).join("");
    ...
    $("genBody").innerHTML =
      ...
      '<label style="display:flex;align-items:center;gap:6px;margin:12px 0 6px;font-size:13px;color:#555"><input type="checkbox" id="expBySub" checked> 按感知子系统分别导出（生成 4 个 CSV，打包为 ZIP）</label>' +
      '<div class="form-actions"><button class="btn-save" onclick="doExportTable()">导出</button></div>' +
      '<div id="expTableResult" style="margin-top:10px"></div>';
    openSheet("sheetGen");
  }
  window.doExportTable = function () {
    var cols = Array.prototype.slice.call(document.querySelectorAll(".expCol")).filter(function (c) { return c.checked; }).map(function (c) { return c.value; });
    var offs = Array.prototype.slice.call(document.querySelectorAll(".expOff")).filter(function (c) { return c.checked; }).map(function (c) { return c.value; });
    if (!cols.length) { toast("请至少选择一列"); return; }
    var allOff = offs.indexOf("") >= 0 || offs.length === 0;
    var labelMap = { name: "名称", office: "管理所", station: "管理站", chan: "段", btype: "建筑物类型", subsystem: "子系统", midType: "中类", subType: "小类", deviceType: "设备类型", ctrl: "是否可控", transmit: "信息传输方式", deviceCode: "设备编码", lat: "纬度", lon: "经度", params: "参数说明", inspect: "巡视次数" };
    ...
    var rows = base.filter(function (b) { return allOff || offs.indexOf(b.office || "未设置") >= 0; });
    if (!rows.length) { toast("没有符合条件的记录可导出"); return; }
    function rowToLine(b) {
      var line = baseCols.map(function (c) {
        ...
        if (c === "office") return normOffice(b.office); // 管理所名称统一
        return b[c] != null ? b[c] : "";
      });
    ...
    function toCsv(list) { return "﻿" + [header.map(csvCell).join(",")].concat(list.map(rowToLine)).join("\r\n"); }
    var bySub = $("expBySub") && $("expBySub").checked;
    if (!bySub) {
      var csv = toCsv(rows);
      var blob = new Blob([csv], { type: "text/csv;charset=utf-8" });
      saveBlobFile(blob, APPNAME + "_建筑物" + (useFilter ? "_筛选" : "") + "_" + getTodayStr() + ".csv");
      $("expTableResult").innerHTML = '<p style="color:#2b8a5d">已导出 ' + rows.length + ' 条记录，' + header.length + ' 列' +
        (useFilter ? '（范围：筛选结果）' : '（范围：全部）') + '。</p>';
      return;
    }
    // 按子系统拆分：每个子系统一个 CSV，整体打包为 ZIP
    busy("正在按子系统拆分并打包…");
    setTimeout(function () {
      var zip = new JSZip();
      var made = 0;
      SUBSYSTEMS.forEach(function (s) {
        var sub = rows.filter(function (b) { return (b.subsystem || "其他") === s; });
        if (!sub.length) return;
        var fn = (APPNAME + "_" + s + "_" + getTodayStr() + ".csv").replace(/[\\/:*?"<>|]/g, "_");
        zip.file(fn, toCsv(sub));
        made++;
      });
      // 另附“全部”汇总
      zip.file((APPNAME + "_全部_" + getTodayStr() + ".csv").replace(/[\\/:*?"<>|]/g, "_"), toCsv(rows));
      zip.generateAsync({ type: "blob", compression: "DEFLATE" }).then(function (blob) {
        idle();
        saveBlobFile(blob, APPNAME + "_按子系统_" + getTodayStr() + ".zip");
        $("expTableResult").innerHTML = '<p style="color:#2b8a5d">已按子系统导出 ' + made + ' 个分组（含汇总），' + header.length + ' 列/组，打包为 ZIP。</p>';
      }).catch(function (e) { idle(); toast("打包失败：" + e.message); });
    }, 30);
  };
  function csvCell(s) {
    s = String(s == null ? "" : s);
