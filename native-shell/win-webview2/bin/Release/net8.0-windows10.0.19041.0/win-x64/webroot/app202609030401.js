...
(function () {
  ...
  function orgValOrDefault(b, k) {
    if (!b) return "";
    if (k === "suo") return normOffice(b.office) || getOrgDefaults().suo || "";
  ...
  function buildMenu() {
    ...
    // v3.42：☆ 收藏开关（阻止冒泡，不触发菜单动作）
    $("menuBody").querySelectorAll(".menu-fav").forEach(function (el) {
      el.onclick = function (ev) { ev.stopPropagation(); toggleFavMenu(el.dataset.k); };
    });
  }
  ...
  function orgPanelDefaults() {
    ...
    var html = '<p style="font-size:13px;color:#555;margin:0 0 8px">新建筑物与导出命名的默认层级值：</p>';
    ORG_LEVELS.forEach(function (L) {
      var opts = levelOptions(L.k), cur = d[L.k] || "";
      if (opts.indexOf(cur) < 0 && cur) opts.unshift(cur);
      html += '<label class="f">默认' + L.t + '</label><select class="f" id="orgDef_' + L.k + '">' +
  ...
  window.orgSaveDefaults = function () {
    var d = {};
    ORG_LEVELS.forEach(function (L) { var el = $("orgDef_" + L.k); if (el) d[L.k] = el.value; });
    setOrgDefaults(d);
    toast("✅ 默认层级值已保存（新建筑物自动带出）");
  };
  // —— 面板③：分类方案管理（v3.42：perc 端 5 scheme 多类型；shuili 端单类型）——
  function orgPanelTypes() {
    if (APPNAME === "水利感知项目一张图") return orgPanelTypesPerc();
    ...
    $("typeAddBtn").onclick = function () {
      ...
      var cs = getCustomTypes();
/* ---------- v3.42：perc 五类分类方案（信息系统分类 / 感知子系统 / 中类 / 子类 / 信息传输方式） ---------- */
  var PERC_TYPE_SCHEMES = [
    { k: "btype",     t: "信息系统分类",   field: "btype" },
    { k: "subsystem", t: "感知子系统",      field: "subsystem" },
    { k: "midType",   t: "中类",            field: "midType" },
    { k: "subType",   t: "子类",            field: "subType" },
    { k: "transport", t: "信息传输方式",    field: "transport" }
  ];
  function percTypesOf(scheme) { try { return JSON.parse(localStorage.getItem("org_perc_types_" + scheme) || "[]"); } catch (e) { return []; } }
  function setPercTypes(scheme, a) { try { localStorage.setItem("org_perc_types_" + scheme, JSON.stringify(a)); } catch (e) {} }
  function percMergedOf(scheme, presentValues) {
    var customs = percTypesOf(scheme);
    var set = {};
    customs.forEach(function (v) { if (v) set[v] = 1; });
    (presentValues || []).forEach(function (v) { if (v && customs.indexOf(v) < 0 && set[v] !== 1) set[v] = 1; });
    return Object.keys(set);
  }
  function orgPanelTypesPerc() {
    var html = '<p style="font-size:13px;color:#555;margin:0 0 8px">感知项目分类方案（5 类）——可独立增删改，重命名对每个受影响设施逐个确认同步。</p>';
    PERC_TYPE_SCHEMES.forEach(function (S, idx) {
      var customs = percTypesOf(S.k);
      var presentSet = {};
      BUILDINGS.forEach(function (b) { var v = b[S.field]; if (v) presentSet[v] = 1; });
      var presentVals = Object.keys(presentSet);
      var dataVals = percMergedOf(S.k, presentVals);
      html += '<div style="border:1px solid var(--border);border-radius:10px;padding:10px;margin-bottom:10px">' +
        '<p style="margin:0 0 6px;font-weight:700;font-size:14px">' + S.t + '（' + dataVals.length + ' · ' + customs.length + ' 自定义）</p><div class="chips">';
      dataVals.forEach(function (v, i) {
        var isCustom = customs.indexOf(v) >= 0;
        html += '<span class="chip percchip" style="display:inline-flex;align-items:center;gap:4px" data-scheme="' + S.k + '" data-i="' + i + '" data-v="' + esc(v) + '">' + esc(v) +
          '<b data-pren="' + S.k + '|' + i + '" style="cursor:pointer;color:#1565c0">✎</b>' +
          (isCustom ? '<b data-pdel="' + S.k + '|' + i + '" style="cursor:pointer;color:#c62828">✕</b>' : '') + '</span>';
      });
      if (!dataVals.length) html += '<span style="color:#999;font-size:13px">（空）</span>';
      html += '</div><div style="display:flex;gap:6px;margin-top:8px"><input class="f" id="percAdd_' + S.k + '" placeholder="添加' + S.t + '名称…" style="flex:1;margin:0">' +
        '<button class="tbtn" data-padd="' + S.k + '">＋ 添加</button></div></div>';
    });
    html += '<div class="form-actions"><button class="btn-save" onclick="orgSaveSync()">💾 保存并同步</button></div>';
    $("orgPanel").innerHTML = html;
    // 添加
    $("orgPanel").querySelectorAll("[data-padd]").forEach(function (b) {
      b.onclick = function () {
        var sk = b.dataset.padd, inp = $("percAdd_" + sk);
        var v = (inp.value || "").trim(); if (!v) { toast("请输入名称"); return; }
        var customs = percTypesOf(sk);
        if (customs.indexOf(v) >= 0) { toast("该名称已存在"); return; }
        customs.push(v); setPercTypes(sk, customs); toast("已添加：" + v);
        orgPanelTypesPerc();
      };
    });
    // 重命名
    $("orgPanel").querySelectorAll("[data-pren]").forEach(function (b) {
      b.onclick = function () {
        var pr = b.dataset.pren.split("|"), sk = pr[0], i = +pr[1];
        var all = percMergedOf(sk, Object.keys((BUILDINGS.reduce(function (o, b) { var v = b[PERC_TYPE_SCHEMES.filter(function (x) { return x.k === sk; })[0].field]; if (v) o[v] = 1; return o; }, {}))));
        var old = all[i];
        var S = PERC_TYPE_SCHEMES.filter(function (x) { return x.k === sk; })[0];
        promptText("修改「" + old + "」为新名称", old, function (nv) {
          nv = (nv || "").trim(); if (!nv || nv === old) return;
          var customs = percTypesOf(sk);
          if (customs.indexOf(nv) >= 0) { toast("该名称已存在"); return; }
          // 受影响设施
          var affected = BUILDINGS.filter(function (x) { return x[S.field] === old; });
          // 旧的如果没有 custom 化，添加；保留：被删除项如不再用
          customs.push(nv); setPercTypes(sk, customs);
          if (!affected.length) { toast("已修改（无设施使用该名称）"); orgPanelTypesPerc(); return; }
          var idx2 = 0;
          var askNext = function () {
            if (idx2 >= affected.length) { orgSaveSync(); orgPanelTypesPerc(); return; }
            var bx = affected[idx2++];
            window.ask("同步设施 " + idx2 + "/" + affected.length,
              "将 <b>" + esc(bx.name) + "</b> 的「" + S.t + "」由「" + esc(old) + "」改为「" + esc(nv) + "」？",
              [{ t: "✔ 确认修改", cls: "btn-confirm2", v: 1 }, { t: "跳过", cls: "btn-cancel2", v: 0 }], function (v) {
                if (v) { bx[S.field] = nv; try { save(); } catch (e) {} }
                askNext();
              });
          };
          askNext();
    ...
    // 删除（仅 custom）
    $("orgPanel").querySelectorAll("[data-pdel]").forEach(function (b) {
      b.onclick = function () {
        var pr = b.dataset.pdel.split("|"), sk = pr[0], i = +pr[1];
        var customs = percTypesOf(sk);
        var v = customs[i];
        var S = PERC_TYPE_SCHEMES.filter(function (x) { return x.k === sk; })[0];
        var affected = BUILDINGS.filter(function (x) { return x[S.field] === v; }).length;
        var doDel = function () { customs.splice(i, 1); setPercTypes(sk, customs); toast("已删除：" + v); orgPanelTypesPerc(); };
        if (affected) window.ask("删除确认", "有 <b>" + affected + "</b> 个设施正在使用「" + esc(v) + "」。<br>删除后下拉/筛选不再显示该名称（设施字段保留原值）。确定删除？",
          [{ t: "确定删除", cls: "btn-exit", v: 1 }, { t: "取消", cls: "btn-cancel2", v: 0 }], function (r) { if (r) doDel(); });
        else doDel();
      };
    });
  ...
  function openFilter() {
    ...
    $("genBody").innerHTML =
      ...
      '<div class="filter-sec"><h4>信息系统分类</h4><div class="chips" id="cTyp">' + chips(types, filters.types, "type") + "</div></div>" +
  ...
  function openEdit(b) {
    ...
    $("editBody").innerHTML =
      ...
      '<label class="f">信息系统分类</label><input class="f" id="fType" list="dlType" value="' + esc(b.btype) + '"><datalist id="dlType">' + opt(types, b.btype) + "</datalist>" +
  ...
  function nearbySearch(restore) {
    ...
    var typeChips = canonicalTypes().map(function (t) {
      return '<span class="chip ntype" data-t="' + esc(t) + '">' + esc(t) + "</span>";
    }).join("");
    var html =
    ...
    $("genBody").innerHTML = html;
    $("genBody").querySelectorAll(".ntype").forEach(function (el) { el.onclick = function () { el.classList.toggle("on"); }; });
    function setMode(m) {
  ...
  window.appDoNearby = function () {
    ...
    if (results.length === 0) {
      html += '<p style="color:#888">范围内无匹配建筑物</p>';
    } else {
  ...
  function deleteBuildingsMenu() {
    ...
    function render() {
      ...
      $("genBody").innerHTML =
        ...
        '</div></div>' +
        '<div class="filter-sec"><h4>信息系统分类</h4><div class="chips">' +
  ...
  // 原生管线匹配：照片落盘（linkPhoto）记录相对路径，彻底规避 localStorage 容量限制
    // v3.42：从 token 中找五级组织名称（按全称优先，再按简称；返回层级与值最强者）
  function matchOrgInToken(token) {
    if (!token) return null;
    var lv = orgStore();
    var levelOrder = ["duan", "zhan", "suo", "guanchu", "ju"]; // 小到大
    var labels = { ju: "局", guanchu: "管理处", suo: "所", zhan: "站", duan: "段" };
    var best = null;
    levelOrder.forEach(function (k) {
      // 末尾追加「所/管理处/局」缀以提高命中率（如 token='京密引水' 先按 guanchu 全称匹配）
      (lv[k] || []).forEach(function (v) {
        if (!v) return;
        if (token.indexOf(v) >= 0) {
          var rank = ({ ju: 5, guanchu: 4, suo: 3, zhan: 2, duan: 1 })[k];
          if (!best || rank > best.rank) best = { level: k, levelLabel: labels[k], value: v, rank: rank };
        }
      });
    });
    return best;
  }
  // 把 item 的 zipName/folder 解析为 orgScope（命中则锁定层+值；多 zip 层级的话用上级优先）
  function itemOrgScope(it) {
    var z = it || {};
    if (z.zipName) {
      var r = matchOrgInToken(z.zipName);
      if (r) return r;
    }
    if (z.folder) {
      // 路径如 "京密引水管理处/水库所"：按 / 拆分，逐段匹配
      var parts = String(z.folder).split(/[\\\/]+/);
      for (var i = 0; i < parts.length; i++) {
        var r2 = matchOrgInToken(parts[i]);
        if (r2) return r2;
      }
      // 整段也匹配一次（兜底）
      var r3 = matchOrgInToken(z.folder);
      if (r3) return r3;
    }
    return null;
  }
  ...
  window.doExportPhotos = function () {
    ...
    var cfgP2 = readExportOptions($("genBody"), "photos");
    var files = [];
    var firstB = null;
    BUILDINGS.forEach(function (b) {
    ...
    var raw = ($("expPhotoName") && $("expPhotoName").value.trim()) || "";
