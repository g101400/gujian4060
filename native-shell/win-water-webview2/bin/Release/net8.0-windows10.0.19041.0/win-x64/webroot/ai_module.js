...
(function (global) {
  ...
  function queryEngine(qtext, progress) {
    return new Promise(function (resolve) {
      try {
        ...
        if (hits.length) { resolve({ source: "local", hits: hits, q: q, orgCtx: orgCtx }); return; }
  ...
  function kbExport() {
    try {
      ...
      // v3.46：导出文件名用纯 ASCII —— Android WebView 下载器会把中文名降为随机 UUID，
      // 导致导出的 zip 无法按原名识别、二次导入失败。统一 kb_<domain>_<日期>.zip。
      var fn = "kb_" + (CFG.domain || "export") + "_" + new Date().toISOString().slice(0, 10).replace(/-/g, "") + ".zip";
      if (!JSZip) { downloadText(fn.replace(/\.zip$/, ".json"), JSON.stringify({ index: o.index, bodies: o.bodies }, null, 2)); return; }
      var zip = new JSZip();
      ...
      zip.generateAsync({ type: "blob" }).then(function (blob) { downloadBlob(fn, blob); })
  ...
  function openKB() {
    ...
    var html =
      '<p style="font-size:13px;color:#3a2e28;line-height:1.6">知识库（Markdown 正文 + JSON 元数据）。可入库格式：①读外部文件(.txt/.md/.csv/.json) ②读网页链接(公众号/微博/普通) ③导出zip ④导入zip。Office(doc/docx/xls/xlsx)与pdf请先另存为 .txt/.csv 再入库。' +
