...
ARCHIVE = os.environ.get("WB_ARCHIVE") or os.path.join(ROOT, "APK归档", "测试包_20260913_v10_int", "win")
...
PROJS = [
    {"key":"shuili","proj":"win-water-webview2","pub":"publish_win_water",
     "name":"水利工程基础信息一张图","exe":"水利工程基础信息一张图.exe","exe_ascii":"ShuiliMap.exe","ver":"3.74.0",
     "appver":"3.74",
     "upg":"{A1B2C3D4-0001-4000-8000-000000000001}"},
    {"key":"perc","proj":"win-webview2","pub":"publish_win_perc",
     "name":"水利感知项目一张图","exe":"水利感知项目一张图.exe","exe_ascii":"ShuiliPerc.exe","ver":"1.50.0",
     "appver":"1.50",
     "upg":"{B2C3D4E5-0002-4000-8000-000000000002}"},
    {"key":"gujian","proj":"win-gujian-webview2","pub":"publish_win_gujian",
     "name":"古建景点打卡","exe":"古建景点打卡.exe","exe_ascii":"GujianMap.exe","ver":"3.7.12.0",
     "appver":"3.7.12",
        ...
        try:
            ...
            if os.path.isfile(out_msi):
                ...
                try:
                    ...
                    ok = "校验异常:%s" % e
                dst = os.path.join(ARCHIVE, "%s_Setup_new4060.msi" % p["name"])
                shutil.copy2(out_msi, dst)
                print("✅ %s_Setup_new4060.msi %d bytes (组件=%d, exe入库=%s) -> %s" % (p["name"], sz, n, ok, dst))
            else:
