...
(function () {
  ...
  function loadSettings() {
    try { return Object.assign({ startPos: null }, JSON.parse(localStorage.getItem("shuili_settings") || "{}")); }
  ...
  var pendingAdd = null;
  var editing = null;
  var filters = { offices: new Set(), stations: new Set(), types: new Set(), photoStatus: "all", photoMin: 3, text: "" };
  ...
  function filterActive() {
    return !!(filters.offices.size || filters.stations.size || filters.types.size ||
  ...
  function openFilter() {
    ...
    $("genBody").querySelectorAll(".chip").forEach(function (el) {
      el.onclick = function () {
        ...
        var set = el.dataset.k === "office" ? filters.offices : el.dataset.k === "station" ? filters.stations : filters.types;
        var v = el.dataset.v;
  ...
  function renderList() {
    ...
    rows.forEach(function (b) {
      html += '<div class="list-card"><span class="go" onclick="appFly(\'' + esc(b.id) + '\')">快速定位 ›</span>' +
        "<h3>" + esc(b.name) + "</h3>" +
  ...
  function showHelp() {
    ...
    $("genBody").innerHTML =
      ...
      "<p><b>⑩ 批量导入建筑物</b>：菜单「批量导入建筑物」→ 选择CSV/Excel/ZIP文件，按列名解析，有经纬度自动定位，无经纬度按名称匹配。</p>" +
      "<p><b>⑪ 定位</b>：菜单「定位我的位置」，如未授权自动跳转设置。</p>" +
      "<p><b>⑫ 导入/导出 ovkmz</b>：兼容奥维互动地图格式，可回灌。</p>" +
      "<p><b>⑬ 导出照片</b>：菜单「导出照片（按管理所）」→ 按管理所分文件夹打包为 ZIP，多张照片自动加序号。</p>" +
      "<p><b>⑭ 导出建筑物表格</b>：菜单「导出建筑物表格」→ 勾选字段列、按管理所筛选，导出为 CSV（Excel 直接打开）。</p>" +
      ...
      "<p><b>底图</b>：天地图矢量/影像，需联网；离线时标记仍在。</p>" +
  ...
  function exportTable() {
    var baseCols = [
      ...
      { k: "chan", t: "段" }, { k: "btype", t: "建筑物类型" }, { k: "lat", t: "纬度" },
      { k: "lon", t: "经度" }, { k: "params", t: "参数说明" }, { k: "inspect", t: "巡视次数" }
    ];
  ...
  window.doExportTable = function () {
    ...
    var labelMap = { name: "名称", office: "管理所", station: "管理站", chan: "段", btype: "建筑物类型", lat: "纬度", lon: "经度", params: "参数说明", inspect: "巡视次数" };
