...
(function () {
  ...
  // 天地图浏览器端 token：优先读取 index.html 中的环境配置，便于不同环境替换；缺省则用内置值
  var TOKEN = (window.TIANDITU_TOKENS && window.TIANDITU_TOKENS.browser) || "61691764ff68bf341c4c9c4770b24b5f";
  var AUTHOR = "小七";
  ...
  function updateXferProgress(phase, done, total) {
    ...
    var label = { copy: "复制照片", zip: "打包", download: "下载", netdisk: "上传网盘" }[phase] || phase;
  ...
  function buildMenu() {
    ...
    groups.forEach(function (grp, gi) {
      $("menuBody").querySelectorAll('.menu-item.sub[data-gi="' + gi + '"]').forEach(function (el) { el.onclick = function () { closeSheet("sheetMenu"); grp.items[+el.dataset.ii].f(); }; });
    });
  }
  ...
  function openFilter() {
    ...
    $("genBody").querySelectorAll(".chip").forEach(function (el) {
      el.onclick = function () {
        if (el.dataset.k === "photoStatus") { filters.photoStatus = el.dataset.v; el.parentNode.querySelectorAll(".chip").forEach(function (c) { c.classList.toggle("on", c === el); }); render(); return; }
  ...
  window.applyFilterAndJump = function () {
    ...
    if (!matched.length) { toast("没有符合条件的选项"); closeSheet("sheetGen"); return; }
    closeSheet("sheetGen");
  ...
  function openEdit(b) {
    ...
    // 照片网格局部刷新（v1.8.2 关键修复）：只更新 phGrid，不重建 input.f 元素，
    ...
    $("fPhotos").onchange = function () {
      ...
      Array.prototype.forEach.call(files, function (file) {
        ...
        r.onload = function () {
          ...
          if (rel) b.photos.push({ file: rel, caption: "" }); else b.photos.push({ data: r.result, caption: "" });
          renderPhotoGrid();
        };
  ...
  function openCheckin(id) {
    ...
    $("ciPhoto").onchange = function () {
      ...
      r.onload = function () {
        ...
        try { if (window.Android && typeof window.Android.storePhoto === "function") editingCheckin.photo = window.Android.storePhoto((id || "checkin_" + Date.now()), file.name, r.result); } catch (e) {}
        if (!editingCheckin.photo) editingCheckin.photo = r.result;
  ...
  window.onPickFiles = function (json) {
    ...
    if (!list || !list.length) { toast("未选择文件"); return; }
    ...
    zips.forEach(function (z) {
      ...
      arr.forEach(function (x) { items.push({ fname: nameNoExt(x.name), base: normalizePhotoName(x.name), path: x.path, folder: x.folder || "" }); });
    });
    runPhotoMatch(items);
  };
  ...
  function linkOrBase64(b, it) {
    ...
    if (window.Android && window.Android.linkPhoto) {
      try { var rel = window.Android.linkPhoto(b.id, it.path, nm + ".jpg"); if (rel) return _push(rel, nm); } catch (e) {}
    }
  ...
  function runPhotoMatch(items) {
    ...
    items.forEach(function (it) {
      var scope = matchScopeForFolder(it.folder);
  ...
  function finishPhotoMatch(pending) {
    ...
    if (!html) { html = '<p style="color:#888">无待确认项。</p>'; }
  ...
  window.dupOverwrite = function (i) { var p = window.__dupBlocks[i]; try { if (p.ep.file && window.Android.deletePhoto) window.Android.deletePhoto(p.ep.file); } catch (e) {} p.b.photos = p.b.photos.filter(function (x) { return x !== p.ep; }); linkOrBase64(p.b, p.it); window.__dupBlocks.splice(i, 1); finishPhotoMatch(window.__dupBlocks.concat(window.__ambBlocks)); };
  window.dupSkip = function (i) { window.__dupBlocks.splice(i, 1); if (!window.__dupBlocks.length && !window.__ambBlocks.length) maybeCleanInbox(); finishPhotoMatch(window.__dupBlocks.concat(window.__ambBlocks)); };
  ...
  function showHelp() {
    ...
    $("genTitle").textContent = "帮助"; $("genBody").innerHTML = html; openSheet("sheetGen");
  }
  function openChangelog() {
    var html = '<div class="changelog-ver"><span class="cv">1.8</span><span class="cd">2026-08-23</span></div>' +
  ...
  /* ---------- 搜索框实时过滤 ---------- */
  $("search").addEventListener("input", function () { filters.text = this.value.trim(); render(); });
  ...
  document.addEventListener("click", function (e) {
    ...
    if (tapCount >= 3) { tapCount = 0; if (addMode) toggleAdd(); else if (measureMode) toggleMeasure(); else if (coordPickMode) { coordPickMode = false; map._container.style.cursor = ""; $("btnMenu").textContent = "☰ 菜单"; } openSheet("sheetMenu"); }
  });
  // 首次进入提示三击快捷键
  setTimeout(function () { var h = $("tripleHint"); if (h) { h.classList.add("show"); setTimeout(function () { h.classList.remove("show"); }, 4000); } }, 800);

  /* ---------- 启动 ---------- */
  load(); initMap(); buildMenu(); render();
})();
