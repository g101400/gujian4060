...
OUT_ZIP = os.path.join(APPLE_DIR, "%s_iOS_%s_可托管_new4060.zip" % (CFG["name"], FILE_VER))
...
def latest_ios_zip():
    ...
    if os.path.isdir(APPLE_DIR):
        for f in os.listdir(APPLE_DIR):
            # 排除本次输出、以及「内部加密」派生包（内含 secure_gate.js / secure/dat.enc.js，
            # 若被当作壳基线会把口令门与加密载荷带进明文包，导致明文 PWA 也要求口令）
            if f.startswith(prefix) and f.endswith(".zip") and f != outbase and "内部加密" not in f:
...
def extract_pwa_shell():
    ...
    for d in ("leaflet", "images"):
        ...
        if os.path.isdir(dp):
            shutil.rmtree(dp)
    # 清掉基线可能残留的加密版专属文件（secure_gate.js / secure/dat.enc.js），
    # 这些只属于「内部加密」派生包，明文包带上会导致启动时误弹口令门
    for fn in ("secure_gate.js",):
        p = os.path.join(STAGE, fn)
        if os.path.exists(p):
            os.remove(p)
    secdir = os.path.join(STAGE, "secure")
