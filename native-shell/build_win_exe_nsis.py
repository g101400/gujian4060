...
for _p in PROJS:
    ...
    _p["ver"] = ".".join(_seg)
    while len(_seg) < 4: _seg.append("0")
    _p["ver4"] = ".".join(_seg)
...
def main():
    ...
    for p in PROJS:
        ...
        if not sd:
            continue
        out_exe = os.path.join(ARCHIVE, "%s_Setup_new4060.exe" % p["name"])
        ...
        if os.path.isfile(out_exe):
            print("✅ %s_Setup_new4060.exe %d bytes -> %s" % (p["name"], os.path.getsize(out_exe), out_exe))
        else:
