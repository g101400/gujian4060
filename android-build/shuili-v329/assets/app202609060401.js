...
(function () {
  ...
  var APP_VERSION = "v3.51";

  var APP_BUILD_DATE = "2026-09-02";
...
function upDoExport() {
  ...
  var name = upCfg.fileTag + "_backup_" + (bundle.appVersion || "v?").replace(/[^\w.]/g, "") + "_" + bundle.exportedAt.slice(0, 10) + ".json";
...
function upOpenImport() {
  upStyle();
  var html =
    '<div class="up-info">选择此前导出的备份文件（<b>' + esc(upCfg.fileTag) + "_backup_*.json</b>）。导入会<b>合并</b>数据：已有记录保留、新记录追加，照片一并恢复，不会覆盖丢失旧数据。旧版备份也可导入新版（字段兼容）。</div>" +
    '<input type="file" id="upImportFile" accept="application/json,.json" style="margin:10px 0;width:100%">' +
  ...
  reader.onload = function () {
    ...
    keys.forEach(function (k) {
      try {
        ...
        if (cur == null) { localStorage.setItem(k, bundle.payload[k]); added++; }
        else { localStorage.setItem(k, upMerge(cur, bundle.payload[k])); merged++; }
      } catch (e) { errs++; }
    });
    if (msg) msg.innerHTML = '<span class="up-ok">导入完成：新增 ' + added + " 项、合并 " + merged + " 项" + (errs ? "，失败 " + errs + " 项" : "") + "。建议重启应用生效。</span>";
    toast("数据导入完成");
  };
...
function upCheck() {
  ...
  fetch(upCfg.manifestUrl, { cache: "no-store" }).then(function (r) { return r.json(); }).then(function (m) {
    ...
    if (cmp > 0) {
      if (msg) msg.innerHTML = '<span class="up-ok">发现新版本 <b>' + esc(entry.version) + "</b>（" + esc(upChannelLabel(ch)) + "）</span><br>" + (entry.note ? esc(entry.note) + "<br>" : "") + (entry.url ? '<a href="' + esc(entry.url) + '" target="_blank" rel="noopener">点击下载更新包</a>' : "安装包未上公开网盘，请从单位内部渠道获取，或联系管理员。");
    } else if (cmp === 0) {
...
upInstall({
  ...
  fileTag: "shuili",
  manifestUrl: "https://3a6c443533764ec8bafd354fae2dfb68.app.workbuddy.link/manifests/shuili.json"
});
