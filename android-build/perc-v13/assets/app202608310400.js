...
(function () {
  ...
  // v1.6.3：增强诊断——尽量抓 err.stack 与脚本 URL，跨域(Script error)时也能暴露更多线索
  ...
  // v1.6.3：ovobj 模块加载自检（ovobj_bridge.js 在 app.js 前引入；若缺失，点「导入 ovobj」会 ReferenceError → Script error）
  setTimeout(function () {
    try {
  ...
  // v1.6.3：老 WebView 兼容 polyfill（安卓 5.x 系统 WebView / 老 WebKitGTK 缺 Array.prototype.find 等 ES6 API）
  ...
  var DEFAULT_FILTER_SEED = { offices: ["水库所"], stations: [], types: [], subsystems: [], midTypes: [], subTypes: [], ctrl: [], transmit: [], coordStatus: "all", photoStatus: "all", photoMin: 3, text: "" };
  function cloneSeed(s) { var o = {}; Object.keys(s).forEach(function (k) { o[k] = Array.isArray(s[k]) ? s[k].slice() : s[k]; }); return o; }
  var SETTINGS = loadSettings();
  function loadSettings() {
    try {
      var s = Object.assign({ startPos: null, defaultFilter: null }, JSON.parse(localStorage.getItem("perc_settings") || "{}"));
    ...
    $("genBody").querySelectorAll(".chip").forEach(function (el) {
      el.onclick = function () {
        if (el.classList.contains("photochip")) {
          ...
          bindPhotoMin();
          saveDefaultFilter(); // v3.38：照片状态变更写回默认值
        ...
        if (el.classList.contains("coordchip")) {
          ...
          saveDefaultFilter(); // v3.38：坐标状态变更写回默认值
          render(); updateFilterCount();
          return;
        }
        ...
        saveDefaultFilter(); // v3.38：管理所/站/类型/子系统变更写回默认值
        render(); updateFilterCount();
      };
    });
  ...
  function renderList() {
    ...
    rows.forEach(function (b) {
      ...
      html += '<div class="list-card" data-id="' + esc(b.id) + '">' +
  ...
  window.appFly = function (id) {
    ...
    // Bug3 修复：必须先按 id 查回建筑对象，否则 b 未定义 → ReferenceError → “运行错误：script error”
    var b = BUILDINGS.find(function (x) { return x.id === id; });
  ...
  function finishPhotoMatch(r) {
    r = r || {};
    save(); render();
    // 关键修复（v1.6.3）：此处【绝不】立即 cleanInbox()。
  ...
  function initAIModule() {
    ...
    window.AIModule.init({
      ...
      fieldSchema: ["office","station","btype","subsystem","model","assetNo"],
      ...
      applyUpdate: function (rec, patch) {
        ["office","station","btype","subsystem","model","assetNo"].forEach(function (k) { if (patch[k] != null && String(patch[k]).trim() !== "") rec[k] = patch[k]; });
        save(); render();
      }
    });
  }
