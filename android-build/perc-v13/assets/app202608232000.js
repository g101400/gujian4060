...
(function () {
  ...
  function initMap() {
    ...
    // 离线包可用性：检测清单是否存在（assets/manifest_offline.json 或原生桥告知）
    OFFLINE_READY = detectOfflineTiles();
    baseVec.addTo(map);
    labelVec.addTo(map);
  ...
  function buildBaseSwitcher() {
    ...
    el.innerHTML =
      ...
      '<label class="bs-opt"><input type="radio" name="baseOpt" value="offline"' + (OFFLINE_READY ? "" : " disabled") + '><span>离线地图' + (OFFLINE_READY ? "" : "（未安装）") + '</span></label>';
  ...
  function switchBaseMap() {
    var order = OFFLINE_READY ? ["vec", "img", "offline"] : ["vec", "img"];
    var idx = order.indexOf(currentBase);
    setBase(order[(idx + 1) % order.length]);
    toast("底图已切换为：" + (currentBase === "vec" ? "矢量地图" : currentBase === "img" ? "影像地图" : "离线地图"));
  }
  // 四期：检测离线瓦片包是否可用
  ...
  function openInspectRoute() {
    ...
    var html = '<p style="font-size:13px;color:#555;margin-bottom:8px">共 ' + all.length + " 次巡视，按巡查时间从新到旧排列：</p>";
    if (!all.length) html += '<div class="empty-tip">还没有巡视记录，去「巡视检查 → 巡视」添加吧。</div>';
    all.forEach(function (it, idx) {
      ...
      html += '<div class="route-item">' +
        ...
        '<div class="rm">巡查：' + esc(it.c.arrivalTime || "—") + (it.c.travelTime ? "　时长：" + esc(it.c.travelTime) : "") + "</div>" +
  ...
  function showHelp() {
    ...
    $("genBody").innerHTML =
      ...
      "<p><b>⑮ 巡视检查</b>：菜单「巡视检查 → 巡视」，选建筑物、拍巡查照片、填巡查时间与巡查情况，自动记入「我的巡视路线」；弹窗/列表/统计页显示已巡视次数。</p>" +
  ...
  var CHANGES = [
    ...
    { v: "v1.1", d: "2026-08-22", items: [
      ...
      "映射说明：古建「打卡」语义对应水利「巡视检查」——设施运行巡查、记录现场状态与处置。"
    ]},
