...
(function () {
  ...
  function openFilter() {
    ...
    $("genBody").querySelectorAll(".chip").forEach(function (el) {
      el.onclick = function () {
        if (el.dataset.k === "photoStatus") { filters.photoStatus = el.dataset.v; el.parentNode.querySelectorAll(".chip").forEach(function (c) { c.classList.toggle("on", c === el); }); saveDefaultFilter(); render(); return; }
        var set = el.dataset.k === "province" ? filters.provinces : el.dataset.k === "city" ? filters.cities : filters.types;
        var v = el.dataset.v;
        if (set.has(v)) set.delete(v); else set.add(v);
        el.classList.toggle("on"); saveDefaultFilter(); render();
      };
    });
    openSheet("sheetGen");
  }
  ...
  function openChangelog() {
    var html = '<div class="changelog-ver"><span class="cv">1.8</span><span class="cd">2026-08-24</span></div>' +
  ...
  function _v33_gujianUpgrade() {
    ...
    // 3) 古建版 tdtKey 设置面板（在 sheetGen 容器中打开）
    window.openTdtKeySettingsGujian = function () {
      ...
      var html =
        ...
        '<label class="f" style="font-size:13px;color:#555">当前密钥</label>' +
        '<div style="display:flex;gap:6px;align-items:center"><input class="f" id="tdtCurVal" readonly value="' + (cur.replace(/"/g, "&quot;")) + '" style="flex:1;font-family:monospace;font-size:12px">' +
        '<button class="tbtn" onclick="tdtPasteCurGujian()" title="复制到剪贴板">📋 复制</button></div>' +
        ...
        '<input class="f" id="tdtNewVal" placeholder="如：61691764ff68bf341c4c9c4770b24b5f" style="font-family:monospace;font-size:13px">' +
        '<div class="form-actions">' +
    ...
    window.tdtPasteCurGujian = function () {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(v).then(function () { toast("已复制到剪贴板"); }, function () { inp.select(); try { document.execCommand("copy"); toast("已复制"); } catch (e) {} });
      } else { inp.select(); try { document.execCommand("copy"); toast("已复制"); } catch (e) { toast("复制失败，请长按输入框选取"); } }
    };
