...
ROOT = "D:/Users/Claw"
NEW_BUILD_DATE = "2026-09-02"
BACKUP_DIR = os.path.join(ROOT, "backup_2026-09-02_before_v349")
ARCHIVE_OLD = "四端安装包_20260901_v348"
ARCHIVE_NEW = "四端安装包_20260903_v349_pub"

SHUILI_DESC = ("v3.49 文档与诊断增强（功能不删不减，与感知 v1.25 / 古建 v3.7 同步）："
...
PERC_DESC = ("v1.25 与水利 v3.49 同步（感知核心=感知切切实设备，保留 subsystem 子系统 + equip_type 设备类型元数据）："
    ...
    "③新增全平台启动诊断（app 启动即记录版本/平台，UOS 另写 launch_err.log）；④UOS 保留 no-store 防缓存 + 每次启动写 launch_err.log/install.log。")

GUJIAN_DESC = ("v3.7 与水利 v3.49 同步（古建核心=景区→园区→子景点→打卡位→标签，key=spot/area/sub/point/tag）："
    ...
    "⑥新增全平台启动诊断（app 启动即记录版本/平台，UOS 另写 launch_err.log）；⑦UOS 保留 no-store 防缓存 + 每次启动写日志。")
...
def main():
    print("🔧 bump_version_v349 — 构建日期 %s" % NEW_BUILD_DATE)
    ...
    print("✅ bump_version_v349 完成。下一步：四端出包。")
